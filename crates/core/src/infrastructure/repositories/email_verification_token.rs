//! `EmailVerificationTokenRepository` の sqlx 実装（SEC6b）。
//!
//! 消費は「UPDATE（未使用・期限内のみ）→ SELECT」の 2 段で行い、`rows_affected == 0` を
//! 使用済み・期限切れ・不存在として扱う（password_reset_token と同じ one-time パターン）。
...
pub struct SqlxEmailVerificationTokenRepository {
    pool: Db,
}

impl SqlxEmailVerificationTokenRepository {
...
fn map_row(row: &MySqlRow) -> Result<EmailVerificationToken> {
...
#[async_trait]
impl EmailVerificationTokenRepository for SqlxEmailVerificationTokenRepository {
    async fn create(&self, token: &EmailVerificationToken) -> Result<()> {
        sqlx::query(
            "INSERT INTO email_verification_tokens (token_hash, user_id, expires_at, used_at) \
             VALUES (?, ?, ?, ?)",
        )
