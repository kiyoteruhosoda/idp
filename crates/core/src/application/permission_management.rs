...
impl PermissionManagementService {
    ...
    pub async fn grant(
        ...
        let code = PermissionCode::parse(code).map_err(|_| {
            PermissionManagementError::Validation(UserMessage::new(keys::PERMISSION_UNKNOWN))
        })?;
        self.ensure_user_in_tenant(tenant, target).await?;
    ...
    async fn ensure_system_admin_change_allowed(
        ...
        {
            Ok(true) => Ok(()),
            Ok(false) => Err(PermissionManagementError::Forbidden(UserMessage::new(
                keys::PERMISSION_SYSTEM_ADMIN_ONLY,
            ))),
