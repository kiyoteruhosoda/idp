...
impl TenantManagementService {
        ...
        self.provisioning
            ...
            .map_err(|e| match e {
                DomainError::Conflict(_) => {
                    TenantManagementError::Conflict(UserMessage::new(keys::TENANT_ALREADY_EXISTS))
                }
    ...
    pub async fn delete_tenant(
        ...
        if tenant.is_root() {
            return Err(TenantManagementError::Forbidden(UserMessage::new(
                keys::TENANT_ROOT_CANNOT_DELETE,
            )));
        }
        ...
        if !grandchildren.is_empty() {
            return Err(TenantManagementError::Conflict(UserMessage::new(
                keys::TENANT_HAS_CHILDREN,
            )));
        }
        ...
        self.tenants.delete(tenant.id).await.map_err(|e| match e {
            DomainError::Conflict(_) => {
                TenantManagementError::Conflict(UserMessage::new(keys::TENANT_NOT_EMPTY))
            }
            other => TenantManagementError::Internal(other.to_string()),
        })?;
...
fn validate_name(name: String) -> Result<String, TenantManagementError> {
    ...
    if trimmed.is_empty() {
        return Err(TenantManagementError::Validation(UserMessage::new(
            keys::TENANT_NAME_REQUIRED,
        )));
    }
    Ok(trimmed)
}
