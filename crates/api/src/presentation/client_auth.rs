...
//! の復号と body（`client_secret_post`）の読み取りをここへ集め、判定は
...
pub fn presented_credentials(
    ...
    Ok(PresentedClientCredentials {
        ...
        body_client_id: body_client_id.filter(|s| !s.is_empty()),
        body_client_secret: body_client_secret.filter(|s| !s.is_empty()),
    })
}
...
mod tests {
    ...
    fn empty_body_fields_are_not_treated_as_presented() {
        let creds =
            presented_credentials(&HeaderMap::new(), Some(String::new()), Some(String::new()))
                .unwrap();
