...
api-audit-invalid-datetime = Invalid date-time in { $field }. Use RFC 3339, e.g. 2026-07-06T00:00:00Z.
...
api-user-email-required = An email address is required.
api-user-email-invalid = Invalid email address format.
api-user-username-too-long = The username is too long. Use at most { $max } characters.
api-user-status-invalid = Invalid status. Choose ACTIVE or DISABLED.
api-user-status-locked-not-allowed = LOCKED cannot be set by an administrator. It is set automatically after repeated sign-in failures.
api-user-self-operation-forbidden = You cannot perform this operation on your own account.
api-register-disabled = Self-registration is disabled for this tenant.
api-register-rate-limited = Too many registration attempts. Try again later.
api-register-verification-invalid = This verification link is invalid or has expired.
api-member-already = This user is already a member of this tenant.
api-member-suspend-not-allowed = Only active guest memberships can be suspended.
api-member-resume-not-allowed = Only suspended guest memberships can be resumed.
api-invitation-invalid-or-expired = This invitation is invalid or has expired.
api-invitation-accept-forbidden = Only the invited user can accept this invitation.
api-permission-system-admin-only = Only holders of idp.system.admin can grant or revoke idp.system.admin.
api-client-secret-public = Public clients do not have a client secret.
api-client-app-name-required = The application name is required.
api-client-redirect-uri-required = At least one redirect URI is required.
api-client-redirect-uri-duplicate = Duplicate redirect URI: { $uri }
api-client-redirect-uri-wildcard = A redirect URI must not contain a wildcard: { $uri }
api-client-redirect-uri-invalid = Invalid redirect URI: { $uri }
api-client-redirect-uri-scheme = A redirect URI must use http or https: { $uri }
api-client-redirect-uri-fragment = A redirect URI must not contain a fragment: { $uri }
api-client-scope-required = At least one scope is required.
api-client-scope-unsupported = Unsupported scope: { $scope }
api-client-scope-duplicate = Duplicate scope: { $scope }
api-client-scope-openid-required = The scopes must include openid.
api-tenant-name-required = The tenant name is required.
api-tenant-has-children = This tenant has child tenants. Delete them first.
api-tenant-root-cannot-delete = The root tenant cannot be deleted.
api-signing-key-already-retired = This key is already RETIRED.
api-signing-key-active-cannot-delete = An ACTIVE key cannot be deleted. Retire it first.
api-client-id-conflict = A client with this client_id already exists.
api-password-too-short = The password must be at least { $min } characters.
api-password-too-long = The password must be at most { $max } characters.
