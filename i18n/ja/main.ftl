...
api-audit-invalid-datetime = { $field } の日時形式が正しくありません。RFC 3339（例: 2026-07-06T00:00:00Z）で指定してください。
...
admin-settings-pending-howto = api → web の順に再起動し、この画面を再読み込みして警告が消えることを確認してください。
...
api-user-email-required = メールアドレスを入力してください。
api-user-email-invalid = メールアドレスの形式が正しくありません。
api-user-username-too-long = ユーザー名が長すぎます。{ $max } 文字以内で入力してください。
api-user-status-invalid = 状態の指定が不正です。ACTIVE または DISABLED を指定してください。
api-user-status-locked-not-allowed = LOCKED は管理者からは設定できません（ログイン失敗が続いたときに自動で設定されます）。
api-user-self-operation-forbidden = 自分自身のアカウントにはこの操作を実行できません。
api-register-disabled = このテナントでは自己登録が無効です。
api-register-rate-limited = 登録の試行回数が多すぎます。しばらく待ってから再度お試しください。
api-register-verification-invalid = この確認リンクは無効か、有効期限が切れています。
api-member-already = この利用者はすでにこのテナントのメンバーです。
api-member-suspend-not-allowed = 一時停止できるのは有効なゲストメンバーシップだけです。
api-member-resume-not-allowed = 再開できるのは一時停止中のゲストメンバーシップだけです。
api-invitation-invalid-or-expired = この招待は無効か、有効期限が切れています。
api-invitation-accept-forbidden = この招待を承諾できるのは招待された本人だけです。
api-permission-system-admin-only = idp.system.admin の付与・剥奪は idp.system.admin の保有者だけが実行できます。
api-client-secret-public = public クライアントにはクライアントシークレットがありません。
api-client-app-name-required = アプリケーション名を入力してください。
api-client-redirect-uri-required = リダイレクト URI を 1 つ以上指定してください。
api-client-redirect-uri-duplicate = リダイレクト URI が重複しています: { $uri }
api-client-redirect-uri-wildcard = リダイレクト URI にワイルドカードは使えません: { $uri }
api-client-redirect-uri-invalid = リダイレクト URI の形式が正しくありません: { $uri }
api-client-redirect-uri-scheme = リダイレクト URI のスキームは http または https にしてください: { $uri }
api-client-redirect-uri-fragment = リダイレクト URI にフラグメントは使えません: { $uri }
api-client-scope-required = scope を 1 つ以上指定してください。
api-client-scope-unsupported = サポートしていない scope です: { $scope }
api-client-scope-duplicate = scope が重複しています: { $scope }
api-client-scope-openid-required = scope には openid を含めてください。
api-tenant-name-required = テナント名を入力してください。
api-tenant-has-children = このテナントには子テナントがあります。先に子テナントを削除してください。
api-tenant-root-cannot-delete = root テナントは削除できません。
api-signing-key-already-retired = この鍵はすでに RETIRED です。
api-signing-key-active-cannot-delete = ACTIVE な鍵は削除できません。先に退役させてください。
api-client-id-conflict = この client_id のクライアントはすでに登録されています。
api-password-too-short = パスワードは { $min } 文字以上で入力してください。
api-password-too-long = パスワードは { $max } 文字以内で入力してください。
