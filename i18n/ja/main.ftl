...
admin-client-field-auth-method-hint = /token・/introspect・/revoke でどう認証するかです。client_secret_basic は Authorization ヘッダを使います（推奨）。client_secret_post は client_id と client_secret をリクエストボディに載せます。private_key_jwt はクライアントが自分の秘密鍵で署名した JWT を使うため、共有シークレットをこちらに保存しません（システム間連携向け）。1 回の要求で複数の方式を提示すると拒否されます。
