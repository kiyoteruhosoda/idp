...
- `0042_tenant_domains`: テナントへドメインを排他的に割り当てる（MT25。ADR-0029）。参加先テナントの
  ログイン画面は「そのテナントの HOME 利用者 ∪ 参加中のゲスト（各自の所属元の登録簿）」を解決するが、
  識別子の一意性は 1 テナントの中でしか担保していない。ゲストは所属元をまたいで集まるので、**同じ
  ユーザー名のゲストが 2 人参加した時点でその 2 人とも参加先の画面から入れなくなる**（fail-closed）。
  一意性をクロステナントへ広げると ADR-0009 §1 のテナント独立が壊れる（あるテナントが `admin` を
  取ると他のどのテナントでも使えなくなる）ため、代わりに `domain` を**グローバルに一意**な列として
  持ち、`local@domain` の形の入力からは所属元テナントを 1 つに決めて、そのテナントの登録簿だけを引く
  （home realm discovery）。1 テナントの中では 1 正規化値が 1 人のものなので（0041）、この経路では
  曖昧さが原理的に起きない。`tenant_id` は `ON DELETE CASCADE` —— テナントが消えたらドメインは
  再び割り当て可能に戻るべきで、消えたテナントの名で押さえ続ける理由がない。`down` は表ごと落とす
  （ドメイン修飾で入っていた利用者は従来の走査側へ戻るだけで、認証は動き続ける）。

- `0043_private_key_jwt`: 機械（人ではない呼び出し元）のクライアント認証に `private_key_jwt` を追加する
  （ADR-0030。RFC 7523）。`clients.token_endpoint_auth_method` の許可値を増やし、検証鍵を置く
  `clients.jwks`（JWK Set。公開鍵のみ）と、assertion の再生を拒む `client_assertion_jtis` を足す。
  共有秘密（`client_secret_*`）は IdP 側にも保存され、クライアント側では設定ファイル・CI の
  シークレットストアに置かれ、リクエストごとに流れる —— 無人で動き続ける機械では漏洩経路が
  積み上がるため、秘密がクライアント側にしか存在しない方式を用意する。検証鍵は**登録済みの
  JWKS だけ**を見る（`jwks_uri` は取りに行かない）—— 認証経路にクライアント側のホスティング障害と
  任意 URL への送信（SSRF）を持ち込まないため。`client_assertion_jtis` に溜まるのは直近の
  assertion 有効期間（アプリ側で 5 分に制限）ぶんだけで、期限切れ行は掃除する。
  expand フェーズのみで既存行は変更しない（既存クライアントは `client_secret_basic` のまま動く）。
...
- `0057_application_identities`: アプリの名乗り（`application_bindings`）を 4 種類へ広げる（ADR-0059）。
  ...
  ⚠ 番号は 0056 を空けてある（同時に開いているテナント設定の PR が 0056 を使うため）。
