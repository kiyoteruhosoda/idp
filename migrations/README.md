...
- `0040_tenant_membership_lookup_idx`: 参加先テナントのゲストを解決する引き方（ADR-0009 §8）に
  ...
  ゲスト解決は「要求テナントの ACTIVE な GUEST」から入るが、既存の索引は PK `(tenant_id, user_id)`
  だけで、先頭列で絞れるのは当該テナントの**全**メンバーまで（HOME 行も同じ表に入る）。この経路は
  所属元での解決が空振りしたとき ＝ 存在しないユーザー名でのログイン試行のたびに走るため、
  索引が無いと認証のホットパスがメンバー数に比例する。`down` は索引を落とすだけ（行は変えない）。
