...
-- ところが `tenant_memberships` の索引は PK `(tenant_id, user_id)` だけで、先頭列で絞れるのは
-- **当該テナントの全メンバー**まで（HOME 行も同じ表に入る）。`membership_type` / `status` は
-- 読んでから捨てることになり、走査量がテナントのメンバー数に比例する。
--
-- この経路が走るのは所属元での解決が空振りしたとき ＝ **存在しないユーザー名でのログイン試行**の
-- たびであり、総当たりが最も送ってくる形でもある。認証のホットパスをメンバー数に比例させないため、
