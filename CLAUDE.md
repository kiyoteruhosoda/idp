...
DDD 4層を Rust モジュールへ写像した単一バイナリクレート（将来 workspace 分割可）。

```
src/
  main.rs             # ブートストラップ・ルータ組立・起動時スキーマ照合
  ...
  infrastructure/     # domain トレイトの実装
    ...
    jwt.rs password.rs crypto.rs clock.rs

  presentation/       # axum ハンドラ・ルータ・DTO
    router.rs handlers/ dto.rs cookies.rs error.rs

migrations/           # sqlx マイグレーション（.sql）
