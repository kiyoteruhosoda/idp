...
- **和文の本文（折り返す文）を中央寄せにしない。** 日本語は単語の切れ目で折り返さないため、
  中央寄せだと 2 行目以降の行頭がそろわず、どこから読むのかが毎行変わる。中央へ寄せてよいのは
  **折り返さないもの**だけ（見出し・ボタン・リンク・入力欄の中身）。
- **補助文言の字色は `--bs-secondary-color`。** 認証画面のラベルは 0.7rem の等幅・字間広めで
  組んであるので、罫と同じ銀（`--assay-silver`）で字を置かない（明度差が足りない）。
...
| 4 | 認可要求の `ui_locales`（OIDC フロー中のみ） | RP が要求する表示言語 |
| 5 | ブラウザ `Accept-Language` | 初回アクセスのみ |
| 6 | システム既定 `ja` | 終端フォールバック |

- `ui_locales` が Cookie より下・ブラウザ言語より上なのは、これが **RP の希望**であって利用者自身の
  選択ではないため。一度でも言語を選んだ利用者の画面を RP の都合で切り替えない一方、何も選んで
  いない利用者にはブラウザ既定より RP の指定を優先する。空白区切りの BCP47 タグを先頭から見て、
  対応言語（`ja`/`en`）が 1 つも無ければ次順位へ落とす（既定へ丸めない）。
