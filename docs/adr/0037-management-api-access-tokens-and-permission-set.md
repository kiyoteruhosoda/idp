...
- **管理コンソールにクライアント権限の画面が無い。** 付与・剥奪は API でできるが、画面は
  まだ無い（`docs/Progress.md`）。
- **`whoami` は `idp.tenant.admin` のまま。** 「管理者としてログインしているか」の判定そのもので、
  細粒度コードでは表せない。細粒度コードだけを持つ利用者はコンソールに入れない（API は使える）。
  コンソール自体を細粒度で出し分けるなら、そのときに設計する。
