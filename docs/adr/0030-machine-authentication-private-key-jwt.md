...
掃除で行が消えた後も受理は続く隙間ができ、そこで同じ assertion を使い回せてしまう。

再生防止が無いと、assertion を一度傍受した相手は `exp` までの間その assertion を使い回せる。
つまり「共有秘密を流さない」という決定 2 の利点が、有効期間の長さだけ目減りする。

併せて `exp` の上限を **5 分**とする（`exp - 現在時刻 > 5 分 ＋ 時計ずれの許容幅` の assertion を
拒否。上限判定にも許容幅を見込むのは、クライアントの時計が数秒進んでいるだけで上限ちょうどの
assertion が落ちないようにするため）。これは再生の窓を短く保つためであると同時に、
`client_assertion_jtis` の保持期間の上限でもある。期限切れの行は掃除して積み上がらないようにする。
