...
- Status: Accepted
- Date: 2026-08-25
- 関連: `docs/adr/0030-machine-authentication-private-key-jwt.md`（機械の認証）、
  `docs/OIDC_INPUT.md` §5（Authorization Code Flow）、G3・G4

## 背景

ADR-0030 で `private_key_jwt` を入れ、機械（CI・バッチ・AI エージェント）が
`client_credentials` で `/token` を叩けるようにした。ところが**その機械を登録できなかった**。
...
さらに `grant_types_for` は常に `authorization_code` を付けていた。機械のつもりで登録しても
...
### 2. redirect_uri を省略できるのは M2M クライアントだけ

空を無条件に許すと、認可フローも M2M も使えない「何もできないクライアント」が作れてしまう。
...
| 機械が単独で API を呼ぶ | **欄ごと非表示** | **非表示（confidential 固定）** | `allow_client_credentials: true`・`redirect_uris: []` |

用途は **api のモデルには無い**。api は `redirect_uris` の有無と `client_credentials` の可否という
2 値を持ち、web がその 2 値へ翻訳する。1 つの決定を 2 つの入力に分けて見せないための、
画面側の語彙である。

機械で `client_type` を隠すのは、confidential 以外あり得ないため（public は秘密を秘匿できず、
`client_credentials` も `private_key_jwt` も成立しない）。**選べないものを見せない。**

検証鍵（JWKS）も `private_key_jwt` を選んだときだけ出す。他の方式で送れば api が拒否するので、
出しておくと必ず失敗する入力欄になる。

### 4. 「両方」は既存クライアントの表示にだけ残す

両方の grant を持つクライアントは、チェックボックス時代の登録で普通に作れた（当時 M2M を
有効にする唯一の道が「redirect_uri を書いてチェックを入れる」だった）。用途を 2 択にしたうえで
そういうクライアントを `user_login` へ丸めると、**編集画面を開いて保存しただけで
`client_credentials` が黙って外れる。**

そこで、両方を持つクライアントを開いたときにだけ選択肢「両方（既存の設定）」を出す。
新規登録では選べない —— 用途ごとにクライアントを分けたほうが、事故時の失効範囲と監査の粒度を
分離できるため。既存を壊さないことと、新しい登録を望ましい形へ寄せることを両立させる。

## 影響

- **M2M クライアントを redirect_uri なしで登録できるようになった。** 手順書の
  `"redirect_uris": []` が実際に動く。
- **機械用クライアントの `grant_types` から `authorization_code` が消える。** `/authorize` は
...
- **用途を api のモデルにも持たせる。** `grant_types` と二重管理になり、「用途は機械なのに
  `authorization_code` を持つ」という不整合を作れる余地が残る。導出できるものは持たない。
- **機械用にダミーの redirect_uri を既定値として入れておく。** 登録は通るが、使わない URL が
  登録済みリダイレクト先として残る。認可フローの入口を開けたままにする点は何も解決しない。
- **用途を 3 択（ログイン／機械／両方）で新規にも出す。** 1 クライアントに 2 用途を持たせる
  登録を勧めることになる。既存の表示にだけ残す（決定 4）。
