...
> **既定は ADR-0015・ADR-0016 で `domain-split`（リッスンポートでサービスを分け、前段プロキシが
> 別サブドメインを割り当てる）へ変更した。** 本節の単一オリジン・パスルーティングは
> `PUBLISH_TOPOLOGY=single-origin` の明示指定で継続サポートする。
...
- **代替**: サブドメイン分割（`web.example.com` / `api.example.com`）＋共有親ドメイン Cookie（`.example.com`）。
  ...
  > 注記（2026-07-25）: この代替は **ADR-0012 で正式サポート化**した（`COOKIE_DOMAIN` 設定時のみ有効。
  > 未設定なら本 ADR の単一オリジン・パスルーティングのまま）。
