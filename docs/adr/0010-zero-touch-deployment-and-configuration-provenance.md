...
ホスト側の標準入口は `deploy.sh` とし、モードは引数なし（デプロイ）、`migration`、`reset` の 3 つに
限定する。`.env` が無い初回デプロイは、自動生成、DB 起動、migration、アプリ起動、readiness 確認まで
実行する。旧 `init.sh` は削除済み（引数なしの `deploy.sh` が初回・更新を兼ねる）。

`reset` はデータ破壊操作なので対象を表示し、自動化用 `--yes` を要求する。通常デプロイと
