...
- **Revised**: 2026-08-23 — §8 の**認証対象テナント**を改訂: パスワード認証を「所属元（HOME）
  テナント限定」から「**当該テナントで ACTIVE なメンバーシップ（HOME / GUEST）を持つ利用者**」へ
  広げる。ゲストが参加先テナントのログイン画面から入れず、所属元テナント ID を URL に打ち込んで
  一度ログインしてから戻る以外に道が無かった（テナント作成者は作成先の ACTIVE GUEST 管理者に
  なる（§4）ため、自分が作ったテナントのコンソールにも直接入れなかった）。パスキー認証は当初から
  この規則で動いており（`ensure_active_member`）、パスワードだけが所属元限定に残っていた。
  §4（権限付与対象）・§8（OIDC フローのメンバーシップ判定）と同じ「ACTIVE なメンバーか」で
  揃える改訂であり、テナント境界は緩めない（メンバーでないテナントでは識別子が解決しない）。
- **Revised**: 2026-08-23（2）— §8 に**所属元テナントの有効性**を明記: 認証・アクセスは「要求
  テナントの ACTIVE メンバーシップ」に加えて「**利用者の所属元テナントが ACTIVE**」を要求する。
  所属元の無効化はその組織の利用を止める操作であり、ゲスト参加先経由の裏口を残す意味ではない。
  併せて、内部 API（`/internal/*`）が `TenantResolver` を通らずテナントの実在・状態を確かめて
  いなかった穴（無効化したテナントの利用者がログインできた）と、トークン発行が所属元テナントの
  状態を見ていなかった穴（発行済みのリフレッシュトークンが参加先テナントで更新し続けられた）を塞ぐ。
- **Revised**: 2026-08-23（3）— §8 に**ログイン識別子の曖昧さの扱い**を明記（MT25）: 解決結果を
  「解決 / 不在 / 曖昧」の 3 値にし、所属元で曖昧だった入力をゲスト解決へ落とさない。併せて
  識別子の一意性を**種別非依存**（1 テナント内で 1 正規化値 1 人。migration 0041）にし、種別違いで
  同じ値を別人が持てる状態そのものを減らす。
- **Revised**: 2026-08-24 — §8 の解決を 3 段へ: 要求テナントの登録簿 → **ドメインで決まった所属元
  テナント**（ADR-0029）→ 参加中のゲストの横断走査。走査は所属元をまたぐため一意性の範囲外で、
  同名のゲストが 2 人参加すると双方が締め出されていた。
- **関連**: `docs/adr/0006-admin-permission-model.md`、`docs/adr/0029-tenant-domains-and-home-realm-discovery.md`、`docs/adr/0007-api-web-service-split.md`、
...
assay は MVP 段階であり、本番運用データは存在しない。スキーマ・マスタデータはマルチテナント対応の
...
- 加えて、**当該テナントで `ACTIVE` なメンバーシップを持つこと**を要求する（判定は 2 段構え）。
  ゲストの一時停止（`SUSPENDED`。§3）は可逆であることが要件で権限行を残すため、権限だけを見る判定では
  停止が管理経路に効かない。SSO セッションはホスト共有で意図的に失効させない（§8）ので、権限行だけを
  見ていると**停止済みゲストが既存セッションで管理 API を使い続けられる**。停止の実効性はこの
  メンバーシップ判定が担い、これは解除（行の削除）に対する二重防御にもなる（権限行の後始末が漏れても
  アクセスは止まる）。
...
- **認証（パスワード・MFA によるログイン）は「当該テナントで `ACTIVE` なメンバーシップ（HOME
  または GUEST）を持つ利用者」に対して行う**（2026-08-23 改訂）。`auth_sessions` に `tenant_id` を
  追加し、`/{tenant_id}/authorize` で開始したフローのテナントを保持する。ログイン画面のユーザー検索は
  **所属元 → 参加先のゲスト**の順で解決する:
  1. 要求テナントの登録簿（`user_login_identifiers` の `tenant_id` が要求テナント）。
  2. 空振りなら、要求テナントに `ACTIVE` な GUEST メンバーシップを持つ利用者を、その利用者の
     **所属元テナントの**登録簿で解決する（ゲストの識別子は所属元に登録されるため）。

  **曖昧なら、そこで止める**（2026-08-23 追記。MT25）。1 が「複数人に当たった」ときに 2 へ進んでは
  ならない。解決結果は `LoginIdentifierMatch`（`Resolved` / `Unresolved(NotFound)` /
  `Unresolved(Ambiguous)`）で返し、2 へ進むのは `NotFound` のときだけとする。`Option<User>` で
  「不在」と「曖昧」を同じ `None` に潰していたため、所属元で曖昧だった入力がゲスト解決へ落ち、
  同じ値を持つゲストが解決されていた（なりすましではないが、「曖昧なら通さない」と「所属元が
  テナント内の名前の取り合いに勝つ」の 2 つが同時に破れる）。利用者への応答は不在・曖昧とも
  資格情報エラーで揃え（存在を露呈させない）、監査には理由を分けて残す（`unknown_user` /
  `ambiguous_identifier`）—— 曖昧は当人が正しいパスワードを出しても入れない状態で、管理者の
  是正が要るためである。登録・改名時の空き判定も**曖昧を「使用中」に数える**（曖昧な値に
  3 人目を足せてしまう）。

  **順序に意味がある。** 1 回でまとめて引くと、同じユーザー名のゲストが参加してきただけで
  「曖昧な入力」になり、そのテナントの HOME 利用者まで締め出される。所属元を先に決めれば、
  参加先で名前が衝突しても割を食うのはゲスト側だけで済む（ゲストは所属元テナントの画面から入れる）。
  ロック・パスワードポリシーは利用者の属性なので変わらず、認証ポリシー（AP2）は**要求テナント**の
  ものを引く（パスキー認証・認可フローの復元判定と同じ扱い。参加先の管理者が自テナントのログインに
  掛けた条件がゲストにだけ素通りすることはない）。停止（`SUSPENDED`）したゲストは `ACTIVE` では
  ないため解決せず、メンバーシップの停止がそのままログインの停止として効く（MT24）。
- **ログイン識別子の一意性は種別に依存しない**（2026-08-23 追記。MT25。migration 0041）。
  1 つのテナントの中で、1 つの正規化値は 1 人のものとする（`user_login_identifiers` の一意キーから
  `identifier_type` を外す）。種別は**正規化のしかた**を決めるためにあり、値の持ち主を決めるための
  ものではない —— ログイン欄は種別を尋ねず、入力を全種別ぶんの読み方へ広げて引くため、種別違いで
  同じ値が存在すると 1 つの入力が 2 人に当たる。追加時の空き判定は元からこれを拒んでいたが、
  無効な行には当たらない（無効化した識別子は解決しない）ため、「A の値を止めている間に B が別種別で
  同じ値を取り、A を有効へ戻すと双方が入れなくなる」窓が空いていた。制約は DB 側に置く。
  **これでも曖昧が完全に消えるわけではない**: 種別ごとに正規化が違う以上、1 つの入力が複数の
  正規化値へ広がる場合（`090-123-4567` はユーザー名なら `090-123-4567`、電話番号なら `0901234567`）
  があり、また参加先テナントのゲスト解決は**所属元をまたぐ**ため一意性の範囲外である
  （同名のゲストが 2 人参加すると双方が締め出される）。上の「曖昧なら止める」はそのために要る。
  恒久策はテナントにドメインを紐づけ、ドメイン修飾された識別子で所属元テナントを 1 つに決めること
  （home realm discovery）で、**ADR-0029 で採用した**（2026-08-24 追記）。解決は 3 段になる:
  要求テナントの登録簿 → ドメインで決まった所属元テナントの登録簿だけ → 参加中のゲストの横断走査。
  2 段目では引くのが 1 テナントだけなので曖昧さが原理的に起きない。裸のユーザー名と未割り当ての
  ドメインは 2 段目に掛からず 3 段目へ落ちるため、**加算であって置き換えではない**。
- **OIDC フロー（authorize/token/userinfo）では「セッションユーザーが要求テナントの `ACTIVE`
  メンバーシップ（HOME または GUEST）を持つこと」を検証**する。ゲストは所属元テナントで確立した
  ホスト共有の SSO セッションでも参加先テナントのフローへ入れる（Entra ID のホームテナント認証の
  簡易形）。メンバーシップのない SSO セッションは当該テナントのフローでは未認証として扱う。
- **認証・アクセスは所属元テナントが `ACTIVE` であることも要求する**（2026-08-23 追記）。
  判定は「要求テナントの `ACTIVE` メンバーシップ」×「利用者の所属元テナントが `ACTIVE`」の論理積。
  所属元テナントを `DISABLED` にすると、そのテナント自身の URL は解決できなくなる（§7）が、ゲスト
  参加先の URL は生きているため、これが無いと「所属元は止めたのに参加先からは入れる」利用者ができる。
  実装は 2 か所: ゲストをログイン欄の入力から引く解決クエリ（`find_active_guest_by_login_identifier`）
  と、メンバーシップ判定（`TenantMembershipRepository::is_active_member`。パスキー認証・SSO 復元・
  管理コンソールのアクセス判定が通る）。
  これは §1 の「テナントの状態は各テナント独立（親の `DISABLED` は子へ伝播しない）」と矛盾しない
  —— §1 は**テナント階層（親→子）**の話で、こちらは**利用者の所属元→その利用者**の話であり軸が違う。
- **トークン発行も所属元テナントの `ACTIVE` を要求する**（2026-08-23 追記）。`authorization_code` /
  `refresh_token` の両 grant が通る共通判定（`TokenService::load_active_user`）で見る。無効化した
  テナント自身の `/{tenant_id}/token` は §7 の middleware が 404 で止めるが、その利用者がゲストとして
  参加している**他テナントの** `/{tenant_id}/token` は生きているため、これが無いと発行済みの
  リフレッシュトークンが寿命（既定 30 日）まで更新し続けられる。この判定はテナント解決の TTL
  キャッシュを通さず毎回 DB を見る（失効に相当する判定で、TTL 分の猶予を作らない）。
- **パスワード再設定も同じ範囲で解決する**（2026-08-24 追記。MT26）。参加先テナントの
  「パスワードをお忘れですか？」からゲストを解決できないと、参加先の画面からログインはできるのに
  再設定だけができない行き止まりになる。解決は**所属元 → 参加中のゲスト**の順で、突き合わせるのは
  `users.email`（メールの届け先そのもの。登録簿は引かない。メールでのログインを有効にしていない
  テナントでも再設定は使えなければならない）。**メールのリンクは所属元テナントを指す** ——
  再設定の実行は「トークンの利用者の所属元 == 要求テナント」を要求するため、参加先を指すリンクでは
  必ず弾かれる。応答は成否によらず常に `Accepted` のままで、列挙は生じない。
  Entra ID も同じ考え方で、リソーステナントでは SSPR させず所属元へ寄せる。
- **内部 API（`/internal/*`）もテナントの実在・`ACTIVE` を確かめる**（2026-08-23 追記）。内部 API は
  テナントプレフィクスを持たないため §7 の `TenantResolver` middleware を通らない。web は DB を
  持たず実在確認を api へ委ねる設計（ADR-0007）なので、api が確かめなければ誰も確かめない。
  `require_internal_tenant` が解決サービス（TTL キャッシュ）越しに検証し、不存在・`DISABLED` は
  資格情報を見る前に 400 で拒否する。**拒否のコードは他の 400 と区別できる固定値**
  （`contracts` の `UNKNOWN_TENANT_ERROR_CODE`）とし、web はこれだけを 404 の画面へ倒す
  （2026-08-24 追記。MT28。区別が無いと、URL のテナント ID を打ち間違えただけで素の 502 になり、
  テナント経路の他の応答と揃わない）。
...
- `/internal/*`（web→api 内部 API）はテナントプレフィクスを付けず、`crates/contracts` の
  ...
  （認証対象は当該テナントの `ACTIVE` メンバーに限られるため、テナント指定のない認証は成立しない）。
...
9. ユースケースの `TenantContext` 対応（OIDC フローのメンバーシップ判定、認証は当該テナントの
   `ACTIVE` メンバー限定）
