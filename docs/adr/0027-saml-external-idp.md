...
- Status: Accepted
- Date: 2026-08-10
- **Revised**: 2026-08-27 — 管理画面の入力の出し分け方を変えた（下記 Consequences の 1 項目）。
...
- 管理 API は `protocol` で入力項目が変わる。SAML では IdP メタデータ XML の貼り付けから
  `entityID` / SSO URL / 証明書を取り込める（既にある `domain::saml_metadata` を
  `IDPSSODescriptor` の解析まで広げる）。
- **管理画面はプロトコルを登録の入口で決める（2026-08-27 の改訂）。** 当初は 1 枚のフォームに
  両プロトコルの区画を並べ、選択に応じて JS で隠していた。**画面から「いま何を登録しているのか」が
  読み取れず**、埋めるべき欄と埋めなくてよい欄が同居していた。いまは
  `/admin/external-idps/new` でプロトコルを選び、`/new/oidc`・`/new/saml` がそれぞれの
  フォームを出す（ADR-0038 が用途で登録の入口を分けたのと同じ形）。サーバが描くのは選ばれた側の
  欄だけになり、**出し分けの JS が要らなくなった** —— JS が動かない環境でも両方を登録できる。
  メタデータの取り込みは SAML の画面にだけ置く（OIDC の discovery は未対応で、取り込む先が無い）。
