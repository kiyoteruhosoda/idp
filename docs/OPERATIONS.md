...
- 発行済みの SSO セッションは失効させないが、次回の判定（ログイン・`/authorize`・管理コンソールの
  アクセス）で止まる。
- **発行済みのリフレッシュトークンは止まらない。** 無効化したテナント自身の `/{tenant_id}/token` は
  404 になるが、その利用者がゲストとして参加している**他テナントの** `/{tenant_id}/token` は生きて
  いるため、そこで発行済みのトークンは寿命（`REFRESH_TOKEN_TTL_SECS`。既定 30 日）まで更新し続け
  られる。すぐ止めるには、参加先テナントごとにゲストメンバーシップを停止する
  （上記「ゲストのアクセスを一時的に止めたいとき（MT24）」）。
