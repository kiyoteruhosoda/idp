...
**普段は何もしなくてよい。** `not_after` の `KEY_ROTATION_LEAD_DAYS`（既定 30）日前になると、
api のバックグラウンドタスク（1 時間おき）が自動で入れ替える。

入れ替えは 3 段で進む。管理コンソールの**署名鍵管理**（`/{tenant_id}/admin/signing-keys`）で
状態が読める（`idp.keys:read` 必須）。

| 段 | 起きること | 画面の表示 |
|---|---|---|
| 1 | 後継鍵を生成し、**JWKS へ公開するだけ**にする | 後継鍵が「公開のみ（署名待ち）」 |
| 2 | `KEY_ROTATION_PUBLISH_LEAD_HOURS`（既定 24）時間後、署名が後継鍵へ移る | 後継鍵が「署名中」 |
| 3 | 次の確認で、追い越された旧鍵が退役する | 旧鍵が「退役済み」 |

**1 と 2 を分けているのが要点。** JWKS をキャッシュしている連携先が新しい `kid` を取り込む猶予に
なる。公開と署名開始が同時だと、キャッシュを持つ連携先はキャッシュが切れるまで検証に失敗する。

**SAML SP にも同じ窓が効く。** IdP メタデータ（`/{tenant_id}/saml/metadata`）は JWKS と同じ集合を
`KeyDescriptor` として並べるので、SP は署名が切り替わる前に後継鍵を取り込める。並びは
**署名中の鍵が先頭**（先頭しか読まない SP の実装があるため）。⚠ 複数の署名証明書を受け入れるかは
SP の実装に依るので、SP を登録するときに挙動を確認すること。

退役しても鍵は `not_after` まで JWKS とメタデータに残る（発行済みトークン・アサーションの検証が
続けられる）。消えるのは `not_after` を過ぎてからで、削除は任意。

### 手動で入れ替えたいとき

同じ画面の「新規署名鍵の生成」で生成する。**既定は自動と同じ 3 段**で、生成した鍵はまず公開だけ
され、24 時間後に署名を引き継ぐ。旧鍵は引き継ぎ後に自動で退役する。

「すぐ署名に使い始める」にチェックを入れると猶予を捨てて即座に切り替わる。**鍵の危殆化のように
待てないときだけ**にする（キャッシュを持つ連携先は、キャッシュを更新するまで検証に失敗する）。

### 退役・削除

- **最後の署名可能な鍵は退役できない**（api が断る）。退役させるとトークン発行が止まるため。
  先に新しい鍵を生成し、署名を引き継がせてから退役させる。
- 削除できるのは退役済みの鍵だけ。**`not_after` が来る前に削除すると、その鍵で署名した発行済み
  トークンが検証できなくなる**（JWKS から消えるため）。急ぐ理由が無ければ残しておく。
- 署名できる鍵が 1 本も無い状態になっても、次のローテーション確認（既定 1 時間おき）で補充される。
  補充する鍵のアルゴリズムは直前の鍵から引き継ぐ。ただし**復旧まで最大 1 時間トークンを発行できない**。

## 外部 IdP でログインできるようにしたいとき

管理コンソールの**外部 IdP**（`/{tenant_id}/admin/external-idps`）で「外部 IdP を登録」を押す
（`idp.tenant.admin` 必須）。まず OpenID Connect と SAML 2.0 のどちらで登録するかを選ぶ。以降の
入力欄は選んだ方式のものだけになる。
...
選ぶか XML を貼り付けると、この 3 つがフォームに転記される（読み込むだけで登録はされない。内容を
確認してから登録する）。**取り込みは新規登録のときだけ**で、編集画面には出ない。
...
- **プロトコルは登録後に変更できない。** 切り替えるには別のプロバイダとして登録し直す
  （既存の連携が別プロトコルの識別子を指したまま残るため）。編集画面にプロトコルの選択欄は無い。
...
- 「主」と付いた行は主たるログイン識別子、「主メール」と付いた行は `users.email` の写しで、
  どちらもここからは変更・削除できない。変えるならプロフィール編集、止めるならアカウントの
  無効化を使う。
- 無効化した識別子は行として残る（削除しない限り、その値が他の利用者へ移ることはない）。
- メールアドレスは利用者の作成時から「主メール」の行として登録簿に在る。**同じ値をもう 1 行
  足すことはできない**（409）。メールでログインさせたいときは次の設定を入れる。

## メールアドレスでログインできるようにしたいとき

設定画面（`/{tenant_id}/admin/settings`）の「メールアドレスでのログインを許可する」を入れる
（`idp.tenant.admin` 必須）。**テナント単位**で、既定は無効。

- 入れると、そのテナントの利用者はログイン欄にメールアドレスを入力しても入れるようになる。
- 切っている間もメールアドレスはテナント内で一意に保たれる（他の利用者が同じ値をユーザー名・
  電話番号・社員番号として取ることはできない）。この一意性は設定に関係なく常に効く。
- 別のメールアドレスでも入れるようにしたいときは、「ログイン識別子」からメール種別で追加する。

## ロックされたアカウントを解除したいとき

管理コンソールの**アカウント一覧**（`/{tenant_id}/admin/accounts`）で、ロック中の利用者には
...
## アプリにサービスアカウントや宛名を結び付けたいとき（ADR-0059）

RP が名簿（`/admin/applications/self/users`）を読めるようにするとき、アプリの API の宛名をアプリの
ものにするとき。

1. 管理コンソール →「アプリ」→ 対象のアプリを開く
2. 「このアプリの名乗り」で種類を選び、相手を入れて「結び付ける」
   - サービスアカウント: そのアプリが assay を呼ぶときのクライアント ID（`client_credentials` だけのもの）
...
## 利用者を作ったあと、本人に使い始めてもらうとき

1. 管理コンソールの `/{tenant_id}/admin/users/new` で利用者を作る。⚠ **パスワードは発行されない。**
   結果画面に**アカウント設定リンク**が一度だけ出る。
2. その画面の「共有する」（スマホなら端末の共有シートが開く）か「コピーする」で、本人へ渡す。
   ⚠ **閉じるとリンクは二度と出ない。** 渡しそびれたら手順 4 で出し直す。
3. 本人はリンクを開き、**パスキーを登録**するか、パスワードを設定する。どちらか一方で終わる
   （終えた時点でリンクは無効になる）。パスキーだけで終えた利用者はパスワードを持たない。
4. 期限（既定 24 時間。`ACCOUNT_SETUP_TTL_SECS`）が切れた・渡しそびれたときは、
   メンバー画面からその利用者の「パスワードを再発行」を押す。新しいリンクが出て、古いリンクは失効する。

⚠ **リンクをチャットで渡してよい。** 開いただけでは消費されないので、プレビューが先に取りに
来ても本人が使える。
...
2. 参加先テナントの管理者が `/{tenant_id}/admin/invitations` を開き、**招く相手のメールアドレス**を
   入力して招待を作成すると、そのアドレスへ承諾リンク付きの招待メールが自動送信される（結果画面に
   送信の成否が表示される）。同じメールアドレスの利用者が複数のテナントにいる場合は「見つかりません」
   になるので、そのときは相手の内部 ID（UUID）を入力する（同じ欄に入れる）。
...
- **管理者が他の利用者のパスワードを再発行する**（本人の操作を待たない）場合は、
  `/{tenant_id}/admin/accounts` で対象利用者の**管理**を開き、「直す」の**パスワード再発行**を
  押す。生成された一時パスワードは画面に一度だけ表示され、本人は次回ログイン時に変更を求められる。

## 利用者を探したいとき（MT22）

1. 対象テナントの管理者で `/{tenant_id}/admin/accounts` を開く。
...
- 一覧に出るのは**そのテナントのメンバー**（HOME と GUEST）で、招待中（未承諾）のゲストも状態
  ...
  操作できず、停止・解除のみ行える）。
- ⚠ **一覧は探す場所で、操作はしない。** 各行の**管理**から 1 人の画面
  （`/{tenant_id}/admin/members/{user_id}`）へ入ると、操作が「見る」「直す」「戻せない操作」の
  3 つに分かれて並ぶ。
...
1. 参加先テナントの管理者で `/{tenant_id}/admin/accounts` を開く。
2. 対象ゲストの行の**管理**を押し、「直す」の**一時停止**を押す。再開するときは同じ画面の
   **再開**を押す。
...
## 利用者を止めたあと、停止の受け口を持たないアプリで手を動かしたいとき

**ここで止めても止まらないアプリがある。** 停止の伝播（back-channel logout。ADR-0053）は、
**RP が受け口を持っていて、その送り先がこのアプリのクライアントに登録されているとき**にだけ届く。
既製品（上流のアプリ）はどちらも持たないので、⚠ **止めた人は、そのアプリのセッションと
アクセストークンが切れるまで入れたままになる。**

対象は **task（Vikunja）** と **forge（Forgejo）** の 2 本（2026-09-20 に使い捨ての利用者で実測）。

| | task（Vikunja） | forge（Forgejo） |
|---|---|---|
| 新しいログイン | 止まる（assay が断る） | 止まる（assay が断る） |
| アプリ側の口座 | **残る**（`Active` のまま） | **残る**（`active: true` のまま） |
| 画面のセッション | **効き続ける** | **効き続ける** |
| API トークン | **効き続ける** | **効き続ける** |
| SSH 鍵 | — | ⚠ この配備は **SSH の口を出していない**（`3000/tcp` だけ）ので、鍵は初めから使えない |

### task（Vikunja）

⚠ **画面に管理者用の口が無い。** CLI で止める（消す）。

```bash
# いま誰が居るか（ID を確かめる）
sudo k3s kubectl -n task-prod exec deploy/app -- /app/vikunja/vikunja user list
# 消す（⚠ --now を付けないと本人へメールを送るだけで、このアプリは mailer を切ってある）
sudo k3s kubectl -n task-prod exec deploy/app -- /app/vikunja/vikunja user delete <id> --now --confirm
```

⚠ **放っておくと最大 3 日入れたまま。** 画面が持つ控え（JWT）は 10 分だが、それを出し直す控えが
Cookie（`vikunja_refresh_token`）にあり、寿命は 3 日ある。**止めたあとでも `POST
/api/v1/user/token/refresh` は新しい JWT を返す**ので、開いているタブがあるかぎり居座れる。

### forge（Forgejo）

管理画面の利用者一覧から無効にする（API なら `PATCH /api/v1/admin/users/{username}` に
`{"active": false}`）。

⚠ **口座を止めるまで、その人が作ったアクセストークンは効き続ける。** セッションを切るだけでは
足りない。

⚠ **新しい人を forge に入れるには、先に口座を作る必要がある**（自己登録は閉じてある）。assay で
利用者を作って割り当てただけでは、SSO で来た人は `/user/link_account` に着いて先へ進めない。

### なぜ受け口を足さないのか

どちらも上流のアプリで、受け口（ADR-0053）と定期照合（ADR-0057 の名簿）を足すには
フォークに手を入れ続けることになる。⚠ **自前のアプリ（7 本）はこの手順を要らなくしてある** ——
足りないのは既製品の側だけなので、**手順で埋める**と決めた（task [#122](https://task.nolumia.com/tasks/122)）。
...
1. 対象テナントの管理者で `/{tenant_id}/admin/accounts` を開く。
2. 対象利用者の行の**管理**を押し、「直す」の**MFA 解除**を押す（所属元が当該テナントの利用者
   のみ。ゲストは所属元テナントの管理者が操作する）。
