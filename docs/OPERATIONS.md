...
2. 自分の構成を指す。スクリプトと同じ場所に **`build-remote-container.env`**（`KEY=VALUE` 形式）を
   ...
   IDP_DEV_CONTAINER=ubuntu-dev            # ビルドを行う dev コンテナ名
   IDP_DEV_USER=sshuser                    # コンテナ内の実行ユーザー
   IDP_DEV_WORKDIR=/work/project/<proj>    # コンテナ内のリポジトリ working dir
   ...
   > そちらが優先される（スクリプト冒頭の既定値を直接書き換えてもよい）。
