## 2026-08-28（3）（テナントを色で見分けられるようにした）

- **テナントごとのアクセント色を足した（`tenants.accent_color`）。** 直前の変更でテナント名は
  ログイン画面と管理コンソールの両方に出るようになったが、**出ているのは文字だけ**で、
  ぱっと見では区別が付かなかった。テナントを渡り歩く管理者（`switch-tenant`）と、複数の組織の
  ログイン画面を見る利用者が、名前を読むまで取り違えに気付けない。
- **出すのはヘッダー上端の 4px の帯だけにした。** 地の明暗（ライト/ダーク）には手を出さない。
  配色は利用者の好み・環境の話、テナント色は「どの組織にいるか」の識別で、役割が違う。混ぜると
  ダークにしている利用者からテナントの色が消えるか、テナント色が可読性を壊すかのどちらかになる。
- **未設定のときは属性ごと出さない。** CSS 側を `var(--idp-accent, transparent)` にしてあるので、
  色を持たないテナントでもヘッダの高さは変わらない（設定した瞬間に 4px ずれるのを避ける）。
  クラスの出し分けが要らないので、`class` 属性を二重に書かずに済む。
- **値は `#rrggbb` の値オブジェクト（`domain::tenant::AccentColor`）で持つ。** この値は HTML の
  `style` 属性へ入るため、書式を型で保証しておかないと検証を通っていない文字列が属性値へ届く
  経路が増える（テンプレートのエスケープは属性値の中身までは見ない）。許可の定義はここが単一の
  出所で、DB の CHECK は長さと先頭文字だけを見る二重防御に留める。書式が壊れた行は「未設定」
  として読む（色の書き間違いでテナントが引けなくなるのを避ける）。
- 編集はテナント設定（`/{tenant_id}/admin/settings`、`idp.tenant.admin`）。表示名の隣に置いた。
  `<input type="color">` は空を送れない（未選択でも `#000000` を送る）ので、解除は併設の
  チェックで表す。api は**未指定＝現状維持・空文字＝解除**で受ける（色の欄を持たない画面からの
  更新が色を消さないように）。
- 運搬は既存の経路に相乗りさせた。コンソールは whoami（テナント表示名と同じ理由で全画面の
  ヘッダに要る）、ログイン画面は `/internal/authorize/login-context`。api 呼び出しは増えない。
