...
- **仮登録の人に、設定リンクの期限を出すようにした**（一覧・1 人の画面）。期限が切れたら札を
  「仮登録・リンク期限切れ」に変える（期限切れかどうかは読んだ時点で判定する。ロック中と同じ）。
  管理 API の `MemberResponse` に `setup_link_expires_at` と `setup_link_expired` を足した。
- 仮登録の人の画面では、「パスワード再発行」を「設定リンクを出し直す」と出す（押して起きることは同じ。
  新しい設定リンクが出て、前のリンクは使えなくなる）。

## 2026-09-25（5）（ホームの見出しと行き先を見分けやすく・取り消しを右端に）

- **管理コンソールのホームで、まとまりの見出しを枠の外の小見出しにした。** 暗い配色では見出し（カードの
  見出し）と行き先の地の色がほぼ同じで、どれが押せるのか見分けられなかった。行き先の行にはアイコンと「›」を付けた。
- **サービスアカウントの管理権限で「剥奪」をいつも右端に置いた。** 長いコードの行だけボタンが次の行の左端へ落ちていた。

## 2026-09-25（4）（検索するとメールアドレスが「[email protected]」になる）

- **web の HTML に `Cache-Control: no-transform` を付けた**（`security_headers`）。Cloudflare の
  「メールアドレスの難読化」が本文中のメールを `[email protected]` へ置き換え、読み込み時のスクリプトで
  戻していたため、ページを読み直さずに一覧を差し替える検索（live-search）の後は置き換えたままの文字が
  出ていた。前段に HTML を書き換えさせないことで直す。ハンドラが付けた指示（`no-store` など）は残す。

## 2026-09-25（3）（アカウント一覧は打つだけで絞り込む）

- **アカウント一覧の検索欄は、「絞り込み」を押さなくても打つだけで反映されるようになった**
  （`assets/live-search.js`）。入力が 0.3 秒止まったら同じ一覧を取り直し、一覧と種別タブの部分だけを
  差し替える（入力欄のフォーカスは外れない。アドレスバーの URL も同じ絞り込みに書き換える）。
  ⚠ 日本語の変換中は送らず、確定してから数える。スクリプトが動かなければ今までどおりボタンで送れる。

## 2026-09-25（2）（旧メモ表の片付け）

- **`tenant_member_notes` を落とした**（migration 0064。ADR-0065 の contract）。入れ替わりの間に旧表へ
  書かれたメモがあれば、落とす前に `account_notes` へ写す。
...
- **管理者が作った利用者は、本人が設定を終えるまで「仮登録」になった**（ADR-0064。migration 0062 の
  `users.pending_setup`）。仮登録は `status` と直交し、`User::is_active` が断るのでどこからも入れない
  （名簿でも `blocked`）。設定リンクでパスキーを登録するかパスワードを決めた時点で外れる。
  ⚠ 忘失時の再設定リンクでは外れず、仮登録の人には再設定リンクも出さない。既に作られて未設定だった
  2 人は移行で仮登録へ倒した。一覧・詳細に「仮登録」と出る。
- **メンバーの「使えるアプリ」をその場で絞り込めるようにした。** アプリ名（全角・半角を問わない）と、
  使える / 割り当てなし / アプリ停止中 の札で絞る（`assets/list-filter.js`）。

## 2026-09-24（2）（利用者の作成と同時にメモを書ける）

- **利用者の作成フォームに管理者メモ欄を足した**（ADR-0063 の続き）。`POST /{tenant_id}/admin/users` に
  任意の `note` を受け、作った利用者の HOME メンバーシップへ付ける。監査は「作った」と「メモを書いた」の 2 件で、
  中身は残さない。⚠ 長すぎるメモは**利用者を作る前に**断る（作ってから断ると「作れたのにエラー」になる）。
  入力エラーで戻しても、書いたメモは消えない。

## 2026-09-24（メンバーの管理者メモ・使えるアプリ・一覧の作り直し・インストール）

- **メンバーに管理者メモを書けるようになった**（ADR-0063）。どういう経緯のアカウントかを残す欄で、
  メンバーシップ（テナント × 利用者）に付く（migration 0061 の `tenant_member_notes`）。ゲストにも書ける。
  一覧にはメモの 1 行目が出て、メモの言葉で絞り込める。⚠ **監査には中身を残さない**（誰が書いたか・
  消したかだけ）。ゲストを解除するとメモも消える。
- **メンバーの画面に「使えるアプリ」が出る**（`GET /{tenant_id}/admin/members/{user_id}/applications`）。
  可否は判定と同じ `Application::admits` で答え、ログインの名乗りを持つアプリだけを載せる。
  「個別」のアプリはその場で割り当て・解除できる。
- **メンバー一覧を「誰か・状態・メモ」の 3 列にした。** 状態は普通でないもの（ゲスト・停止中・無効・
  ロック中など）だけを札にし、操作列は落として行全体を 1 人の画面への入口にした。
- **管理コンソールをスマホのホーム画面へインストールできるようになった。** テナントごとのマニフェスト
  （`/{tenant_id}/admin/manifest.webmanifest`。認証の外）と 192 / 512px の PNG アイコンを置いた。
  service worker は置かない。

## 2026-09-21（4）（生成パスワードを返す口が無くなった）

- **管理者によるパスワード再発行の応答からも `generated_password` を落とした**（ADR-0062 の仕上げ）。
  作成に続いて再発行も**設定リンクだけ**を返す。⚠ **assay が管理者へ「使えるパスワード」を渡す
  経路は 1 つも無くなった。**
- 再発行はいまも値を生成して置き換える（古い値を通らなくするため）が、⚠ **その値は誰も知らない。**
- 非常時の道具（deploy-repo の `host/breakglass`）は設定リンク経由へ移して配布済み
  （deploy-repo #73。本番の使い捨て利用者で 1 往復を実測）。
- ⚠ **「既知のパスワード × `must_change_password`」は管理 API では作れない**ので、その状態を要る
  試験は DB へ直に入れる（`support::force_password_change`）。

## 2026-09-21（3）（アカウントは本人が使えるようにする）

- **利用者を作っても、管理者に生成パスワードを渡さなくなった**（ADR-0062）。代わりに
  **本人へ渡すワンタイムリンク**（`setup_url`）を一度だけ返す。作成時の `password_hash` は
  誰も知らない値で、本人がリンクでパスキーを登録するかパスワードを設定するまで誰も入れない。
- **リンク先はパスキーが主**。⚠ **パスワードを一度も持たないアカウントが運用できる**
  （パスキーのログイン経路は `must_change_password` を見ない）。使えない端末のために
  パスワードの設定も畳んで置いてある。
- ⚠ **開いただけでは消費しない。** チャットのプレビュー bot は相手より先に URL を取りに来る
  （過去に 1 本食われた）。消費は資格情報が実際に決まったときだけ。
- 渡し方は端末の**共有シート**（`navigator.share`）とコピー。管理者の結果画面に並ぶ。
- トークンは忘失時の再設定と同じ表で、`purpose` 列（`reset` / `setup`）で分ける
  （migration 0060）。⚠ **`reset` のリンクではパスキーを登録させない** —— メールを一時的に
  読める相手が居座る手段になるため。
- 管理者のパスワード再発行も同じリンクを返す（いまのパスワードの即時無効化は従来どおり）。
  ⚠ `generated_password` は非常時の道具（deploy-repo の `host/breakglass`）のために残してあり、
  そちらが移り次第落とす（`docs/Progress.md`）。
- 寿命はテナント設定 `ACCOUNT_SETUP_TTL_SECS`（既定 24 時間）。

## 2026-09-21（2）（ゲストはメールアドレスで招く）

- **ゲスト招待の相手を、メールアドレスでも指せるようにした**（ADR-0061）。これまでは内部 ID（UUID）
  だけで、⚠ **参加先の管理者は相手のテナントの名簿を見られない**ため、その値は assay のどこにも
  出てこなかった（相手に自分の ID を調べて送ってもらうしかなかった）。
- 欄は 1 本のまま。UUID として読めれば内部 ID、そうでなければメールアドレスとして引く
  （読み分けは `InvitationService::resolve_invitee`）。API の項目名は `invitee`、旧名 `user_id` も
  引き続き受ける。
- ⚠ **メールで引くのはテナントを跨いでちょうど 1 人のときだけ**（`users.email` の一意性はテナント内
  だけ）。0 人でも複数人でも同じ「見つかりません」を返し、曖昧だったことは**言い分けない**
  （言い分けると他テナントの利用者の存在だけが漏れる）。逃げ道の「内部 ID でも指定できます」は
  画面の説明とエラーの両方に出す。
...
- **アプリを `DISABLED` にすると、その名乗り（`service_account`）のサービスアカウントに `client_credentials` の
  トークンを出さない**（ADR-0060）。既定のトークンも、割り当てのある他のアプリの宛名のトークンも
  `unauthorized_client`（監査の `reason` は `own_application_disabled`）。
- ⚠ **管理トークンだけは出す。権限コードは載せない。** 通るのは自分の名簿（self）だけで、名簿は全員 `blocked`
  を返す ——RP の定期照合が止まったことを読めるように残した。
- 名乗り・割り当て・権限コードは消えない。`ACTIVE` に戻せば次のトークンから戻る。発行済みは寿命まで有効。
...
- **設定画面に「絞り込む」を付けた**（ユーザーの要望）。名前・キー・説明の文字で、テナントの設定と
  全体のランタイム設定の両方を絞る。テナントの設定は「このテナントで決めたものだけ」、ランタイム設定は
  「テナントが決められるもの／要対応／再起動待ち」でも絞れる（後者は一覧を見られる root にだけ出す）。
- 既存の一覧（メンバー・監査ログ）と同じく **GET のフォームでサーバ側が絞る**。スクリプトは足していない。
  絞っているときは各区画に「表示中: M / N 件」と「絞り込みを解除」を出す。⚠ 再起動待ちの警告は
  **絞る前の一覧**から作る（絞ったせいで警告が消えないように）。

## 2026-09-16（11）（アプリを使う主体に、サービスアカウントを並べる）

- **アプリを使う主体を「人」と「サービスアカウント」の 2 種類にした**（`0059`。ADR-0059 の決定 5・6）。
  `application_assignments` に `id`（UUIDv7）と `kind`（`USER` / `SERVICE_ACCOUNT`）を足し、
  `user_id` か `client_id` のどちらか 1 つだけが埋まる形にした。既存の人の割り当ては値を変えずに `USER` になる。
- ⚠ **種類で振る舞いが違う。** 人の割り当て＝ログインしてよい、サービスアカウントの割り当て＝**そのアプリの
  宛名のトークンを取ってよい**。「全員（`EVERYONE`）」に含まれるのは人だけ。名簿（self）と件数
  （`assigned_count`・「誰も入れません」）も人だけを数える。code の発行時の判定は人だけを見る（変更なし）。
- **宛名のトークンは「宛名のアプリ」と「割り当て」で出す。** 宛名が有効・あるアプリの `resource` の名乗り・
  そのアプリが有効・要求元がそのアプリに割り当てられている、の全部を満たすときだけ `aud` に載る。
- ⚠ **クライアントへの宛名の貸し出し（`client_resources`）を消した。** 同じことをアプリの外に持つだけに
  なっていた。移行で「宛名を名乗るアプリ」への割り当てへ写し、⚠ **どのアプリの名乗りでもない宛名の
  貸し出しがあれば、何も変えずに止まる**（推測で写さない）。管理 API の `/admin/clients/{id}/resources` と、
  クライアント詳細の「許可した宛先」も消した。
- 管理 API: `POST .../assignments` は `kind`（`user` / `service_account`）必須。外すのは
  `DELETE .../assignments/users/{user_id}` と `DELETE .../assignments/service-accounts/{client_id}`。
  アプリの詳細は `assigned_service_accounts` を返す。
- 管理コンソールのアプリ詳細の「このアプリを使う主体」を 1 つの一覧にした（人のシルエット「人」/
  サーバーの形「サービス」。アイコンだけにせず必ず文字を並べる）。追加の入口も 1 つで、押した先で種類を選ぶ。

## 2026-09-16（10）（テナントのメール送信を設定画面から入れる）

- **管理コンソールの設定画面に「このテナントのメール送信（SMTP）」の区画を足した**（ADR-0058 §8）。
  `idp.smtp:read` が無ければ区画ごと出さない（api が 403 を返すかで決め、web は権限を判定しない）。
- 「IdP 全体の送信設定を使っています / このテナント専用の送信設定を使っています」を出し分け、専用の
  設定があるときだけ「IdP 全体の送信設定に戻す」を出す。⚠ 全体に従っているときは欄が空で出る
  （api が全体の宛先・利用者名を返さない）。
- パスワード欄は**空欄なら現行維持**、消すのは「保存するときにパスワードを消す」を付けたときだけ
  ——空欄を消去と読むと、他の項目だけを直して保存したときにパスワードが消える。

## 2026-09-16（9）（アプリの名乗りと、名簿の self）

- ⚠ **名簿の口の向きを直した。** ADR-0057 の `GET /{tenant_id}/admin/applications/oidc/{client_id}/users`
  は、RP が**自分の `client_id` を経路に書いて**聞き、`idp.applications:read`（テナントの全アプリの
  名簿が読める）で通していた。「このサービスアカウントはどのアプリのものか」を RP に申告させる形で、
  しかも広すぎた。判断は ADR-0059。
- **アプリの名乗り（`application_bindings`）を 4 種類にした**（`0058`）——ログイン用（`oidc`）・`saml`・
  **サービスアカウント**（`client_credentials` だけの client）・**API の宛名**（`resource`）。
  どれも 1 つのアプリにだけ属し、別のアプリの名乗りなら 409 でそのアプリ名を返す。種類と相手の性質を
  突き合わせる（ログイン用の client をサービスアカウントとして結び付けない。逆も同じ）。
  列の名前は `protocol` から `kind` へ改めた。
- **名簿は `GET /{tenant_id}/admin/applications/self/users` で引く。** 宛先は呼び出し元の
  サービスアカウントの名乗りから assay が決める。⚠ **権限コードでは通さず**、結び付いていない主体
  （利用者・名乗りの無いサービスアカウント）には **403**（空の名簿を返さない）。応答の形は ADR-0057 のまま。
  旧口は消した。
- 名乗りを持つサービスアカウントには、**権限コードが 0 件でも管理トークンを出す**（`perms` は空。
  通るのは self の名簿だけ）。
- ⚠ **移行 0054 がサービスアカウントに作った「自分だけのアプリ」を消した。** サービスアカウントは
  アプリではなく、アプリの名乗りである。割り当て・認証ポリシーからの参照・他の名乗りのどれかが
  付いていれば、移行は**止まる**（黙って消さない）。どのアプリの名乗りにするかは推測せず、人が結び付ける。
- 管理コンソールのアプリの詳細に「このアプリの名乗り」を置いた（種類を選んで結び付ける）。

## 2026-09-16（8）（テナントごとのメールの経路）

- **テナントが自分の SMTP を持てるようにした**（ADR-0058 §8）。持っていなければ全体の経路で送る。
  招待は招待するテナント、メール検証は検証するテナント、email OTP は利用者の所属元、パスワード再設定は
  ログイン画面のテナントの経路で送る（⚠ 再設定だけは利用者を引く前に決める ——所属元で決めると
  「使えない / 受け付けた」の違いから利用者の有無が漏れる）。
- ⚠ **経路は塊で解決する。** テナントが host・利用者名・パスワード・差出人のどれかを持っていれば
  **そのテナントの塊だけ**を使い、欠けた項目を全体の値で埋めない。項目ごとに落とすと、テナントが
  `host` だけを自分のサーバに向けるだけで**全体の SMTP パスワードがそのサーバへ送られる**（試験で固定）。
  port と TLS の行だけでは「持っている」に数えない（空のフォームを保存しただけでメールが止まらないように）。
- **ADR-0051 §3 を覆した。** `idp.smtp:read` / `:write` はどのテナントでも付与でき、届くのは
  scope のテナントの経路だけ。⚠ `idp.tenant.admin` は今までどおり**含意しない**。⚠ 全体の経路の口
  （`/admin/system-settings/smtp`）は今までどおり **root でなければ 403**。
- 口は `GET` / `PUT` / `DELETE /{tenant_id}/admin/settings/smtp`。経路を持たないテナントには
  `inherited: true` と空の項目を返し、⚠ **全体の宛先や利用者名は見せない**。パスワードは暗号化して
  `tenant_settings` に置き、平文は返さない。
- 権限の説明文を migration 0057 で改めた。設定画面はまだ（T58。テナント設定の画面の後）。

## 2026-09-16（7）（テナント設定の画面と API）

- **テナント管理者が、自分のテナントの設定を API と画面から決められるようにした**（ADR-0058）。
  口は `/{tenant_id}/admin/settings/tenant/keys`（一覧）と `…/keys/{key}`（`PUT` で決める・
  `DELETE` で全体に戻す）。権限は `idp.tenant-settings:read` / `:write`。消費側は参照のたびに
  テナントの値を引くので、保存した値は再起動なしで効く（TTL キャッシュは書いたときに捨てる）。
- **項目はキー定義から導く。** ⚠ **キーを足すたびに API の型や画面を書き足さない** ——一覧は
  `scope = TenantOverridable` の定義から並び、入力欄は値の型（`kind`）と選択肢（`choices`）で決まる。
- **値だけを見せない。** 各項目に、効いている値・出どころ（`TENANT_OVERRIDE` / `INHERITED`）・
  全体の値を返し、画面も「全体の値に従っています」と「このテナントで決めた値です」を並べる。
  ⚠ **「全体の値に戻す」は行を消す**（同じ値を書くのとは違い、以後は全体の変更に追随する）。
- **テナントの権限で全体のキーを開かない**（二重に止める）。付与の側は `idp.tenant-settings:*` が
  `idp.system.admin` を含意せず、全体の設定の口は完全一致でしか通らない。使う側はテナントの口でも
  `scope = Global`・定義に無いキー・秘匿値のキーを書き込みも解除も断る。
- **締め出し得る値は保存の前に確認を挟む。** 判定はキー名ではなく定義の側に置いた
  ——値の型に `SettingKind::Choice` を足し、選択肢ごとに `locks_out` を持つ。
  `AUTH_POLICY_DEFAULT_EFFECT`（`deny`）と `APPLICATION_ASSIGNMENT_ENFORCEMENT`（`enforce`）に
  付けた。api は `confirmed` が無ければ 409 を返し、画面は確認の 1 画面を挟んで送り直す
  （JavaScript に頼らない）。
  ⚠ 副作用: この 2 キーは**全体の設定でも選択肢以外の値を保存時に断る**ようになった
  （これまでは保存でき、次の起動で `Config` が落ちていた）。
- **全体の設定画面に、テナントが上書きできる各キーの「従っているテナントの件数」と「既定から外れて
  いるテナント」を出した**（ADR-0058 §6。干渉はしないが見えなくはしない）。テナントをまたいで読む
  口はリポジトリに 1 本だけ足し（`list_overrides_across_tenants`）、秘匿値は SQL の時点で落とす。
  ⚠ 数えるのはキー定義で `TenantOverridable` のキーだけ。同じ表に置く SMTP の行（T58）は混ざらない。
- 監査ログ `tenant_settings.updated` を足した。⚠ 値は記録せず、キーと設定/解除の別だけを残す。
## 2026-09-16（6）（テナントの設定で動かす）

- **テナントが上書きできる 14 キーを、消費側で `TenantSettingsService` から引くように作り替えた**
  （パスワードポリシー 4・ロックアウト 3・`SSO_IDLE_TTL_SECS` / `SSO_ABSOLUTE_TTL_SECS` /
  `STEP_UP_MAX_AGE_SECS`・招待 / 再設定 / メール検証のリンクの寿命と `PASSWORD_RESET_CONSOLE_LINK_ENABLED`）。
  判断は ADR-0058（§13 を追加）。⚠ **行は 1 つも作っていないので、挙動は変わらない**（全テナントが全体に従う）。
- ⚠ **`Config` からこの 14 キーを消した。** getter が残っていると「そこだけ全体の値で動き、静かに
  間違える」ので、読めない形にした。組み込み既定は定義の `default_value` の 1 か所だけになり、
  型の合わない全体の値で起動しない fail-fast は `Config` の起動時の検査として残した。
- **どのテナントの値か**: パスワード・ロックアウト・SSO セッション・step-up・再設定 / 検証リンクは
  利用者の**所属元**、招待リンクは**参加先**。⚠ URL のテナントでは決めない（ゲストが入口ごとに別の
  規則で数えられる）。SSO セッションの idle 延長も確立と同じ所属元の値で延ばす。
- web へ返す SSO Cookie の `Max-Age` は、⚠ **確立したセッションの絶対期限から採る**（応答側で設定を
  引き直さない）。パスキーの検証結果に所属元テナントを載せた。
- 14 キーの `restart_required` を false にした（参照のたびに引くので再起動なしで効く）。全体の値を
  保存したら解決器の全体キャッシュを捨てる。
- 漏えい照合器は常に組む（使うかはテナントごとに決まる）。外向きの通信は照合を有効にしたテナントでしか起きない。
- テナントを持たずに値を引く経路は無かったので、解決器は `TenantId` を必須にした（黙って全体へ落ちる入口が無い）。
- 試験で固定したこと: 2 テナントで違う値を入れて互いに効かないこと、行の無いテナントが全体に従うこと
  （パスワードの最小長・履歴・ロックの閾値・SSO の寿命・step-up の間隔・リンクの寿命・コンソール出力）、
  ゲストが参加先から失敗しても所属元の閾値で数えること、組み込み既定が降ろす前の `Config` と同じ値であること。

## 2026-09-16（5）（ポリシーの既定動作と割り当ての強制をテナントが決める）

- **`AUTH_POLICY_DEFAULT_EFFECT` と `APPLICATION_ASSIGNMENT_ENFORCEMENT` をテナントが上書きできる
  キーにした**（ADR-0058 §4）。名簿が整ったテナントから `enforce` へ倒せる。⚠ **値は変えていない**
  （既定は `allow` / `record_only` のまま。行の無いテナントは全体の値に従う）。
- **判定する側は `Config` を読まない。** 認証の 8 経路（ログイン・MFA・パスキー・外部 IdP・パスワード
  変更・SSO 復元・管理コンソール・ポータル）と割り当ての門（`ApplicationAccessService`）は、
  注入されたトレイト `AccessDecisionSettings` から**参照のたびに**テナントの値を引く ——⚠ 1 経路でも
  起動時の値を読み残すと、その経路だけ全体の値で動いて静かに間違えるので、型で塞いだ。
- **再起動は要らなくなった**（`restart_required = false`）。全体の値を保存したら全体のキャッシュを捨てる。
- **web へ配るのをやめた**（`shared_with_web = false`。ADR-0058 §10）。認証ポリシー画面の既定動作は、
  api の一覧の応答に足した `default_effect`（そのテナントの値）を描く ——⚠ web はテナントごとの設定を
  持たず、ADR-0013 の起動時スナップショットの不変条件はそのまま。`APPLICATION_ASSIGNMENT_ENFORCEMENT`
  は web のコードから一度も読まれていなかったので、配るのをやめただけ。
- ⚠ 設定が引けない（保存値が壊れている）とき、既定動作は**ログインを断る側**（500）、割り当ては
  **記録するだけ**に倒す ——割り当ての門は、問い合わせの失敗と同じく全員を締め出さない。
- 試験は 2 テナントに違う値を入れて書いた（core の単体・web の画面・api の DB を使う統合）。

## 2026-09-16（4）（テナント設定の受け皿）

- **テナントごとの設定を置く受け皿を入れた**（`tenant_settings`。migration 0056）。判断は ADR-0058。
  ⚠ **この段ではまだ誰も読まない** ——挙動は 1 つも変わらない（消費側の作り替えは別のリリース）。
- **解決の口は 1 本だけ**（`TenantSettingsService`）: テナントの行 > 全体の行 > 環境変数 > 組み込み既定。
  ⚠ **テナントに行が無いことが「全体に従う」**で、既定値の写しを配らない。⚠ 全体の行も起動時の
  スナップショットから採らない ——スナップショットは DB 由来と環境変数由来を区別しないので、
  全体の上書きを消したときに再起動まで古い値が残る。
- **キー定義に「単位」を足した**（`SettingScope`）。⚠ **定義の必須項目にした**ので、キーを足す人は
  必ず「全体か、テナントか」を選ぶ。テナントが上書きできるのは 14 キー（パスワードポリシー 4・
  ロックアウト 3・セッション 3・リンクの有効期間 4）。⚠ `AUTH_POLICY_DEFAULT_EFFECT` と
  `APPLICATION_ASSIGNMENT_ENFORCEMENT` はまだ入れていない ——`shared_with_web` が立ったままで、
  web への配り方を直す段と一緒でないと画面が既定値を表示する。
- 値の型の検査を全体とテナントで 1 つの関数に寄せた（`validate_setting_value`）。
- 試験で固定したこと: トークンの寿命・発行者・Cookie・`AUTH_SESSION_TTL_SECS`・テナント解決の
  キャッシュの寿命はテナントへ降ろさない／テナントが上書きできるキーは web へ配らない／定義の既定と
  `Config` の既定がずれていない（⚠ ずれると、**行の無いテナントだけが全体と違う値で動く**）。

## 2026-09-16（3）（アプリの名簿を引く口）

- **RP の定期照合（ADR-0049 の I7 の 3 つ目）が読む口を足した**
  ——`GET /{tenant_id}/admin/applications/oidc/{client_id}/users`。判断は ADR-0057。
  back-channel logout（受け口）と停止の配信（ADR-0053）で取りこぼしたぶんを、RP が定期的に
  拾い直すための一覧である。⚠ **これまで「IdP の利用者一覧と突き合わせる」と書きながら、
  その一覧を引く口が無かった**（`/admin/members` は `sub` を返さず、`current-users` は画面用で
  ページングが無い）。
- **答えるのは「このアプリを使ってよい利用者」**で、テナントの利用者一覧ではない。割り当ての
  モードに関わらず同じ形で返し、⚠ **モードは RP に知らせない**（`EVERYONE` ならテナントの
  メンバー、`INDIVIDUAL` なら割り当てのある利用者）。
- **宛先は OIDC の `client_id`。** ⚠ アプリの内部 ID を持たせると、**作り直した日に RP は古い
  ID のまま静かに別の名簿を読む**。解決は判定と同じ経路（アプリ ↔ binding ↔ client）を通る。
- **載せるのは `sub` と 3 つの状態だけ**（`allowed` / `blocked` / `unknown`）。⚠ 理由を細かく
  返すと**割り当てのモードが漏れる**うえ、RP の行動は変わらない。プロフィールは載せない。
- ⚠ **`updated_since`（差分）は入れない。** 外した割り当ては行ごと消えるので差分に出てこず、
  足した割り当ては `users.updated_at` を動かさない ——**照合がいちばん拾いたい変化が、いちばん
  伝わらない**。照合は全件で回し、消息は `subs=` で聞く（一度に 100 件まで）。
- ⚠ **止めたアプリは「空」ではなく「全員 `blocked`」で返る。** 「誰も使えない」と「引けなかった」
  を取り違えさせないため。binding が無い client も同じ理由で **404**（空の名簿を返さない）。
- 状態を決める規則は判定（`CodeIssuanceService`）と同じ `Application::admits` を通る
  ——⚠ 規則が 2 つあると「名簿に居るのに入れない」が起きる。SQL は事実だけを返す。

## 2026-09-16（2）（アプリという段を足す）

- ⚠ **assay に「アプリ」という実体が無かった。** あったのはプロトコルごとの設定が 2 本
  （OIDC の `clients` 23 件 / SAML の `saml_service_providers` 0 件）、互いを参照せずに並んでいる
  だけで、**同じアプリが OIDC と SAML の両方で繋がる**という普通の形が表せなかった。結果、認証
  ポリシー・同意・権限・宛名・停止の配信は**すべて OIDC 側にしか付かず**（SAML のアプリには MFA を
  要求できない）、「誰がどのアプリを使ってよいか」を決める場所がどこにも無かった（RP 7 本が
  それぞれ自前の名簿で断っていた）。判断は ADR-0054。
- **`applications` / `application_bindings` / `application_assignments` を足した**（`0054`）。
  既存の `clients` は**アプリ 1 件 ＋ OIDC binding 1 件**へ開き、生成した行の `id` には元の
  `clients.id` をそのまま使う（移行が決定的・冪等になり、どの client から開いたかを追える）。
  ⚠ **割り当てにロールの列は持たせない** ——載るのは「使ってよいか」の 1 ビットだけ。
- **判定は `CodeIssuanceService::issue` に置いた** ——ログイン・MFA・パスキー・外部 IdP・パスワード
  変更・同意・SSO 復元の 7 経路が必ず通る、**code を発行できる唯一の経路**である。経路ごとに判定を
  書き写すと、経路が 1 つ増えるたびに書き忘れが門を開ける。
- ⚠ **いきなり断らない。** `APPLICATION_ASSIGNMENT_ENFORCEMENT`（既定 `record_only`）で、まず
  「割り当てが無いのに来た人」を監査ログへ溜める。切り替えは**設定**で行う（コードの入れ替えに
  すると、戻すのにデプロイが要る）。監査は `application.access_denied` 1 種別で、通したか断ったかを
  `result` で分ける ——切り替えの前後を同じ問い合わせで数えられる。
- **断りは RP へ戻さず assay の画面で伝える。** ⚠ **「サインインはできましたが」から始める**
  ——判定は認証の後なので、利用者からは「入れたのに弾かれた」ようにしか見えず、黙って弾くと障害と
  区別が付かない。アプリ名と、管理者へ頼む導線を添える。
- **管理コンソールに「アプリ」を足した。** 登録は「全員 / 個別」を選ばせ、⚠ **既定は「個別」**
  （絞り忘れが全員に開いたままになるのを防ぐ）。「自分を割り当てる」は初めからチェック。
  「全員」のアプリには**いま入れている人**を出し、そのまま名簿へ写せる。⚠ 「個別」で名簿が空の
  まま保存するときだけ確認を挟む。
- **認証ポリシーの条件を `application_ids` へ寄せた**（`0055`）。寄せない限り SAML のアプリは
  ポリシーの外に残る。本番は 1 行だけで `client_ids` を使っている行が無く、移すデータは無かった。
- **新しく登録した連携先も、その場でアプリとして開く。** ⚠ 開かないと、登録しただけの RP が
  **全員に開いたまま**になり、しかもそれが画面のどこにも出ない。

## 2026-09-16（1）（ゲスト招待を画面から承諾できるようにする）

- ⚠ **ゲスト招待は、誰も画面から承諾できなかった。** 被招待者が所属元テナントでログインした状態で
  招待リンクを開いて承諾しても、「先に所属元テナントでログインしてください」へ戻り、
  メンバーシップは `INVITED` のまま残っていた ——**ログインは済んでいるのに、である。**
- 原因は web の中継。承諾でも SSO セッションを **そのテナントの管理トークンへ交換してから**
  api を呼んでいた（`api_client.rs` の `admin_send_no_content`）。⚠ **被招待者はまだメンバーでは
  ないので交換は必ず失敗し**、401 が「未ログイン」の画面に写っていた。承諾は**メンバーになるための
  操作**なので、メンバーであることを前提にできない。
- 承諾だけ SSO Cookie を**そのまま転送する**送信経路にした。api 側の口は前から Cookie を
  資格情報に受けている（`AuthenticatedUser`。権限は要求しない。ADR-0009 §3）ので、api は変えていない。
- ⚠ **層ごとの試験はどちらも通っていた**（api の統合試験・core の単体試験）。
  **web → api の並びを被招待者で通す**試験が無く、隙間に落ちていた。
  `crates/web/tests/invitation_accept.rs` を足し、⚠ **管理トークンの交換が 401 を返す状況**で
  承諾が通ることを固定した（交換に依存し直すと落ちる）。
- 実測（break-glass の ROOT 参加で踏んだ。deploy-repo のタスク #72）: 無効なトークンでも同じ画面に
  なることで「トークンではなくセッションの受け取りで止まっている」と切り分け、
  api へ Cookie を直接添えると 400（トークン無効）まで進むことで確定した。

## 2026-09-15（9）（アクセストークンの寿命を 5 分にする。ADR-0056）

- `ACCESS_TOKEN_TTL_SECS` を**この配備では 300 秒**と決めた（組み込みの既定 900 は変えない）。
  値は宣言（deploy-repo の `k8s/idp-prod/20-config.yaml`。stg も同値）に書く。
- ⚠ **これまで「決めた結果」ではなく既定のままだった。** 環境変数にも `system_settings` にも
  行が無く、900 秒で動いていた。
- 決め手は**失効の速さ**だけ ——RP は自前のセッションを持ち、更新のたびに assay を引かないので、
  ⚠ **寿命を延ばしても「assay が落ちている間に入れる人」は増えない**。止めた利用者・
  クライアントのトークンが通り続ける上限が **15 分 → 5 分**になる。
- ⚠ **機械トークン（`client_credentials`）にも同じ寿命が乗る。** 使い手（nolumiawiki /
  mp3play）は期限の 60 秒手前で取り直すので、取り直しが 14 分ごと → 4 分ごとになる。
- ⚠ **`userinfo` の呼び出しは監査に記録されない**（該当する種別が無い）。「誰が引いているか」は
  監査ログからは数えられず、今回は RP の実装を読んで数えた。

## 2026-09-15（8）（test_deploy.sh を同時に流しても落ちないようにする）

- ⚠ **`scripts/test_deploy.sh` が中間出力を固定の `/tmp/deploy-*.out` へ書いていた**（31 種類・96 行）。
  同じ容れ物で 2 本流すと、片方が出力を書き直している最中にもう片方がそれを grep し、**毎回違う行で落ちる**。
  ⚠ **落ちた側の変更とは無関係**なので、自分の変更のせいに見える。CI は 1 本ずつ走るので起きない。
- 中間出力を実行ごとの作業場所（`mktemp -d` の `$TMP`）の下へ移した。作業場所自体は以前から
  実行ごとに作っていて、**出力だけが外に出ていた**。
- 成功したら片付ける。⚠ **失敗したときは残して場所を出す** ——中間出力を消してしまうと、
  落ちた理由を手元で追えなくなる（実行ごとに別の場所なので、残しても次の実行とぶつからない）。
- CI に関門を 1 つ足した（`scripts/*.sh` に固定の `/tmp/` を書かない。行頭の注記は除く）。
- 実測: ずらして 3 本同時に流すと、直す前は **24 本中 7 本**が落ちた（158・168・302 行と、
  落ちる行は毎回違う）。直したあとは同じ条件で **24 本中 0 本**。

## 2026-09-15（7）（ログアウトでは RP へ知らせない。ADR-0055）

- RP-initiated logout で back-channel logout の通知を積むのをやめた。RP へ知らせるのは
  管理者が利用者を止めたときだけ（ADR-0053）。
- ⚠ **codelab のサインアウトが、同じブラウザで SSO で入った他の RP のセッションまで閉じる
  ところだった。** SSO セッションは複数の RP で共有されているため。これまで表に出なかったのは
  `backchannel_logout_uri` を登録した RP が 1 つも無かったから（本番 24 件中 0 件）で、
  停止の通知を効かせるために登録する前に塞いだ。
- `LogoutResult` から `backchannel_targets` / `user_sub` / `sid` を落とした。配送キューと
  `sid` の運搬（ADR-0024）は残る。

## 2026-09-15（6）（メンバーの操作を 1 人ずつの画面へ移す）

- ⚠ **一覧の 1 セルに操作を 7 つ並べていた。** 操作列は 168px しかないのに「パスワード再発行」は
  152px あり、**ボタンが縦 7 段に積まれて 1 行が 282px**、幅の広いものは**右端で切れていた**
  （表の中身 996px に対し器が 934px で 62px はみ出し）。⚠ **7 つを 1 セルに収める前提そのものが
  無理だった** ——列幅を調整しても直らない。
- **`GET /{tenant}/admin/members/{user_id}` を足し、操作をそこへ集めた。** 一覧は
  「探す・状態を見る」だけになり、各行には**管理**への導線が 1 つだけ載る。
- 詳細では操作を**重さで 3 段**に分けた ——見る（移動するだけ）／直す（やり直せる）／
  ⚠ **戻せない**（削除・解除は赤枠で隔離）。一覧では「権限を見る」と「削除」が同じ大きさで
  並んでいた。
- ⚠ **api に「1 人を引く」口が無かった。** 一覧はページングされているので、目的の 1 人が
  何ページ目に居るかは web には分からない。`GET /admin/members/{user_id}` を足した
  （`TenantMemberFilter` に `user_id` を追加。他テナントのメンバーは 404）。
- 一覧から**表示名の列**を落とした ——ユーザー名と同じ値になる人が多く、無い人は「-」が
  並ぶだけで人を見分ける助けになっていない（詳細には出る。絞り込みは今までどおり氏名も見る）。
- **列見出し「状態」を「メンバーシップ」に改めた。** ⚠ スマホのカード表示では
  「状態 ACTIVE」の真下に「アカウント 無効」が並び、**別概念の 2 つが矛盾して読めていた。**
- 既存の試験 5 本（MFA 解除・トークン再発行・ロック解除・停止/再開・確認文を属性で渡すこと）は
  **弱めずに詳細画面へ付け替えた**。⚠ **一覧に操作が戻らないこと**を見る試験を 1 本足した。
- ⚠ **1 人の画面が 500 を返していた（E2E が捕まえた）。api と web の両方で同じ取り違えをしていた。**
  経路 `/{tenant_id}/admin/members/{user_id}` には**パス変数が 2 つ**あるのに 1 つで受けており、
  web 側は `WrongNumberOfParameters` で**ハンドラへ入る前に 500**、api 側は
  `Path<Uuid>` が
  **先頭のテナント ID を取っていた**ため必ず「メンバーではない」になっていた。⚠ **同じ経路の
  既存ハンドラは api も web も最初から 2 つで受けている** ——揃えていなかったのは新しい 2 つだけ。
- ⚠ **見落とした理由は、新しい口に「経路を通る」試験を付けていなかったこと。** テンプレートを
  直接描く試験は通っていたのに、実際の経路は 500 だった。api に
  `one_member_can_be_fetched_by_id`（DB あり）、web に `admin_members_console.rs`
  （api を wiremock で差し替え、ルータ経由で一覧・1 人の画面・不明な相手の 3 本）を足した。
- E2E も**消さずに新しい道筋へ通した**（一覧 → 1 人の画面 → 権限）。⚠ 手元の `cargo test` は
  通っていたのに CI が赤になったのは、**E2E が一覧に権限リンクが出ることを見ていた**ため。
  `docs/OPERATIONS.md` の手順 5 箇所（ロック解除・パスワード再発行・一時停止・MFA 解除・
  利用者を探す）も「行の**管理**を押してから」に直した。

## 2026-09-15（5）（TOTP の自己登録を通す）

- ⚠ **TOTP の登録は本番で一度も成功していなかった。** `otpauth://totp/{issuer}:{account}` は `:` で
  区切るため issuer に `:` を含められないが、TOTP の issuer へ OIDC の issuer
  （`https://identity.nolumia.com`）をそのまま渡していた。totp-rs が URI を組めず、api は
  `internal`、web は 500 を返す。**issuer が URL である限り必ず失敗する**ので、`user_authenticators`
  に `totp` の行が 1 つも無かった。ログインの TOTP 検証は URI を組まないため影響なし。
- **issuer から見出しへの変換を `TotpRegistrationService::new` の中に閉じた。** 呼び出し側で
  落とす作りにすると、渡し忘れた 1 か所で登録が丸ごと止まる（実際そうなっていた）。
- ⚠ **QR の宛名が空だった。** web は利用者名を知らず空文字を渡していたので、認証アプリの一覧で
  どの口座か見分けられない。api が SSO セッションの利用者から引くようにし、
  `InternalTotpSetupRequest.account_name` は契約から外した。
- ⚠ **エラー画面に出口が無かった。** 500 の画面にリンクが 1 つも無く、左上の名乗りもリンクでは
  ないので、ブラウザの戻る以外に手が無い。テナントが分かるときだけ戻り先を出す（コンソール配下は
  コンソールのホーム、それ以外はアカウント設定）。**今いる場所と同じなら出さない** ——押しても
  また同じエラーになるだけで、出口に見えて行き止まりになる。
- ⚠ **web は api の内部エラーを黙って 500 にしていた**（27 か所と step-up のゲート）。web のログに
  残るのは tower_http の `response failed / Status code: 500` だけで、**どの呼び出しで落ちたかが
  分からない**。`handlers::api_internal_error(call)` へ集約して `call=<呼び出し名>` を必ず残し、
  分岐が増えても穴が開かないよう `every_internal_result_that_becomes_a_500_says_which_call_failed`
  がソースを走査して固定する（ルート一覧の検査と同じ理由で、実物と別の一覧を持たない）。
- **この経路には試験が 1 本も無かった。** api 側は実 DB で **QR の中身まで**見る ——200 が返った
  だけでは今回の失敗は捕まえられない。リカバリーコードも同じ並びで追加した（こちらは元から正常）。
- ⚠ 当時の **api 側**に `totp setup internal error` が出ていなかった点は未解明。本番と同じ層の
  重ね方・同じ `RUST_LOG` で組んだ試験
  （`telemetry::tests::an_application_error_reaches_both_stdout_and_the_capture_sink`）では、
  アプリの ERROR は stdout と `log` テーブルの両方へ届いている。

## 2026-09-15（4）（メニューの箱を「向き」で分ける）

- ⚠ **「連携先・鍵管理」が向きの逆のものを混ぜていた。** 連携先（OIDC / SAML）・宛先・署名鍵は
  assay が **ID を渡す**側（出ていく）だが、外部 IdP と認証ポリシーは人が **assay に入る**ときの
  話（入ってくる）。とくに認証ポリシーは「誰がどの要素でログインできるか」で、中身は完全に
  アカウント側である。⚠ **同居していたせいで探しても見つからなかった**（この箱に 11 項目中
  6 項目が集まり、2 列も釣り合っていなかった）。
- **外部 IdP と認証ポリシーをアカウント側へ移し、箱の名前を「アカウントとサインイン」にした。**
  ⚠ **「アカウント」のままにしない** ——認証ポリシーを探す人がその見出しを開かない。
- **並びを「何を管理するか」が先、「何が起きたか」が後にした**（アカウントとサインイン →
  連携先・鍵管理 → 運用・監査 → システム・テナント設定）。管理しに来た人が毎回
  運用・監査を読み飛ばしていた。4 / 4 / 2 / 1 になり、2 列の釣り合いも取れる。

## 2026-09-15（3）（管理コンソールのメニューを 1 か所から描く）

- ⚠ **サイドバーとホームのカードが食い違っていた。** サイドバーにだけ「外部 IdP」が、ホームに
  だけ「認証ポリシー」が載っており、**どちらも相手の部分集合にならない**ので、利用者からは
  「片方は省略している」とも読めない見え方になっていた。同じ 11 項目を 2 つのテンプレートへ
  手で書いていたのが原因で、片方へ足したときにもう片方を忘れた形。
- **定義を `templates::CONSOLE_NAV` の 1 か所に置き、両方をそこから描くようにした。**
  ⚠ **ずれを「書けなく」してある** ——テンプレートが `/admin/…` を直に書くと
  `the_console_menu_is_the_only_place_that_lists_admin_screens` が落ちる。
  両方に同じ行き先が出ることは `the_sidebar_and_the_home_cards_offer_the_same_screens` が固定する。
- ⚠ **`admin-nav-external-idps-desc` が両言語とも未定義だった。** 「外部 IdP」はサイドバーに
  しか無く、サイドバーは説明を出さないので誰も定義していなかった。ホームへ載せた途端にキー名が
  そのまま画面へ出るところだった（`Messages::get` は見つからないキーをそのまま返す）。
  未定義を `every_menu_entry_has_both_a_name_and_a_description` が検出する。
- **見た目 2 点。** まとまりの説明を**カード見出しの副題**へ移した ——項目リストの中に置くと
  字下げも幅も項目と同じで「押せない 1 行目」に見えていた。あわせて `h-100` を外した
  ——`card-body` が余った高さを吸うため、⚠ **項目 1 つのカードでその項目が底へ押し出され**、
  説明が主役で行き先がおまけに見えていた（「システム・テナント設定」で約 280px の空白）。

## 2026-09-15（2）（RP 統合ガイドを置く。ADR-0049 G10）

- **`docs/RP_INTEGRATION.md` を追加した。** assay につなぐ側（RP）が守ることを 1 か所に集めた
  ——結び付きの鍵は `(iss, sub)` でメールアドレスではないこと、`state` と PKCE が必須で
  `nonce` は任意なこと、`acr` は**要求したら確かめる**こと、プロフィールの写しは毎回上書きして
  RP では編集させないこと、停止を届かせる 3 つ（受け口・短命のアクセストークン・定期照合）。
- ⚠ **7 本の RP が少しずつ違う作りになっていた原因はここ。** `docs/OIDC_INPUT.md` は MVP の
  設計書で、つなぐ側の作り方は書いていなかった（ADR-0049 G10）。
- 「動くが壊れている形」を落とし穴として並べた。`logout_token` を `sid` 必須で書くと
  **管理者による停止が素通りする**（ADR-0053 で `sid` を載せないため）など。

## 2026-09-15（利用者を止めたことを RP へ伝える。ADR-0053）

- **管理操作で失効させたセッションを、back-channel logout で RP へ知らせるようにした。**
  ⚠ これまで通知を送っていたのは**本人がログアウトしたときだけ**で、管理者が利用者を
  無効化しても・パスワードを再発行しても・MFA を解除しても、assay の中で SSO セッションと
  トークンが消えるだけだった。RP は自分の発行したトークンで動くため気付けず、
  **止めた利用者が RP 側では入ったまま**になっていた。
- 起点は「セッションを消した場所」に置いた（`UserLifecycleService::revoke_credentials` /
  `revoke_credentials_strict`）。⚠ **呼び出し側に「無効化のとき」と書いていない** ——
  失効を伴う操作が次に増えたとき、足し忘れても何も壊れずテストも通ってしまうため。
- 管理操作が失効させるのは**その利用者の全セッション**なので `sid` は載せない。
  載せると RP は名指しの 1 つしか閉じず、別端末のセッションが残る。
- 積めなかったときの扱いは呼び出し元に合わせた。無効化・パスワード再発行は fail-open
  （操作は成立させ、ERROR で残す）。MFA 解除は**失効しきったときだけ**知らせる
  （失敗したまま「止まった」と伝えると、RP は閉じたのに assay に残る）。
- ⚠ **受け口を持たない RP には届かない。** 受け口は RP 側の実装
  （ADR-0049 G3。7 本へ導入済み）。

## 2026-09-14（別名のメールを主メールへ昇格させる。ADR-0052）

- **登録簿に在る `email` 種別の行を、1 手で主メールにできるようにした**
  （`POST /{tenant}/admin/users/{user_id}/login-identifiers/{identifier_id}/primary-email`。
  管理コンソールの一覧にもボタンが出る）。ADR-0050 で追加のメールは登録簿へ足せるように
  なったが、**足したものを「主」にする導線が無かった** ——付け替えるにはプロフィール編集で
  アドレスを打ち直すしかなく、打ち間違えれば一覧に無い 3 つ目のアドレスが主メールになる。
- 昇格は**入れ替え**である。いまの主メール行の降格・選んだ行の昇格・`users.email` の差し替えを
  同じトランザクションに置く（順番も `primary_email_of_user` の UNIQUE に合わせて固定）。
  ⚠ **`PATCH` の項目にしていない。** 項目にすると `is_primary_email: false`（＝宛先の無い利用者）
  が要求として書けてしまい、必ず断ることになる。
- 昇格できるのは**その利用者のメール種別の行だけ**（種別の判定はアプリ層と SQL の両方に置く）。
  昇格した行は必ず有効にする ——主メール行は識別子単位で有効化できないので、無効なまま昇格
  させると二度と有効にできない行が通知の宛先になる。
- もとの主メール行は**消さずに別名として残す**（消すとその値を他人が取れてしまう）。
  `email_verified` は動かさない（管理者によるメール変更と同じ扱い）。監査は
  `user.primary_email_changed` に種別だけを残し、アドレスは残さない。
- スキーマは変えていない（`primary_email_of_user` は migration 0052 で入っている）。

## 2026-09-13（3）（SMTP 設定だけを機械から入れられるようにする。ADR-0051）

- **`idp.smtp:read` / `idp.smtp:write` を足し、`GET/PUT /{tenant}/admin/system-settings/smtp` を
  切り出した（migration 0053）。** SMTP を設定しないとパスワード再設定のメールが送れず、
  送れない間は管理者が初期パスワードを手渡しすることになる。ところがその設定を入れる口は
  `idp.system.admin` の完全一致で、**そのコードはクライアントへ付与できない**（ADR-0037 §5）。
  結果として SMTP パスワードは人が画面へ手で貼るしかなく、**秘密を運ばせないために作った仕組みが
  秘密を運ばせていた。**
- 分割するのは SMTP の 1 枚だけ。再起動・テナント作成・SMS・ランタイム設定は今までどおり
  `idp.system.admin` の完全一致だけが通る。口と応答の型も分けてある（既存の
  `PUT /admin/system-settings` は SMS とランタイム設定も同じ本文で受けるので、**権限だけ細くしても
  触れる範囲は細くならない**）。パスワードの平文は今までどおり返さない。
- ⚠ **この 2 つは root scope でしか持てない。** `system_settings` はテナント列を持たない
  システム全体の表で、テナントの中で配れるようにすると、そのテナントの管理者が全テナントの
  メール経路を変えられる。`idp.tenant.admin` は含意せず、付与の側（`is_grantable_in_tenant`）と
  使う側（エンドポイントの root 判定）の両方で落とす。
- 管理コンソールの SMTP 欄は変えていない。**機械の口は人の口を置き換えない**
  ——メールが出せない状態のときに、復旧の手段が機械 1 本になるのは避ける。

## 2026-09-13（2）（メールアドレスを登録簿へ移す。ADR-0050）

- **`users.email` と登録簿が同期していない穴を塞いだ（migration 0052）。** 他人のメールを
  識別子として足す向きはアプリ層（`ensure_available` の段 3）が拒んでいたが、**逆向き**
  ——管理者が A のメールを、B がユーザー名・電話番号・社員番号として持つ値へ変更する
  ——は素通りだった（`update_profile` は `users` 側の一意制約しか見ない）。塞いである側も
  規則であって制約ではなく、同時実行の窓が残っていた。
  `users.email` を登録簿の**主メール行**（`primary_email_of_user`）として写し、利用者作成・
  プロフィール編集と同じトランザクションで同期する。以後、同じテナントの中では 1 つの
  正規化値は 1 人のもので、メールはユーザー名・電話番号・社員番号とも衝突しない。
- **メールでのログインをテナント単位のスイッチにした**（`tenants.email_login_enabled`。既定
  無効）。行は移行で全利用者ぶん入るが、**当てただけでは入り口は広がらない** ——
  migration 0029 が警告していた「認証の入り口が黙って広がる」を起こさずに一意性だけ先に効かせる。
- 主メール行は識別子単位で変更・削除できない（主識別子と同じ。消すと `users.email` と割れる）。
  一覧では「主メール」の見出しで示し、操作ボタンを出さない。
  **⚠ ついでに主識別子の行も直した** —— AP15b で `id` が必ず付くようになったのに出し分けが
  `id` の有無のままで、押しても必ず失敗するボタンが並んでいた。
- **テストの `mark_email_verified` が並列実行でデッドロックしていたのを直した。** 結合したまま
  UPDATE すると `user_login_identifiers_primary_uk` を走査して共有ロックを取りながら `users` の
  行の排他ロックを待ち、同時に走る登録（`users` を握ったまま登録簿へ INSERT）と循環する。
  引いてから主キーで更新する 2 文に分けた。

## 2026-09-13（メンバー一覧にユーザー名を出す）

- **メンバー一覧にユーザー名（主たるログイン識別子）の列を足した。** これまで出ていたのは
  メールアドレスと氏名だけで、**利用者が「入れない」と言ってきたときに何を打ってもらえば
  よいのかが管理画面から読めなかった**。値は AP15b 以降の正本である登録簿の主識別子行
  （`primary_of_user`）の表示値を相関サブクエリで引く（`users.preferred_username` は
  migration 0039 で撤去済み）。付けずに作られたアカウントは `-`。
- **絞り込みの対象にユーザー名を足した。** 一覧に出ている値で引けないと「見えているのに
  探せない」列になる。`EXISTS` で主識別子行だけを見るので、無効化した別名には当たらない。
...
- **TOTP コードのリプレイを塞いだ（migration 0051）。** これまで TOTP は検証（skew=1・step=30 秒）を
  通すだけで**受理したコードを消費済みとして残していなかった**ため、正しい 6 桁は有効な時間窓
  （最大 ~90 秒）の間、別セッション・別 IP からでも再利用できた（RFC 6238 §5.2 違反）。受理した
  time-step を `user_authenticators.totp_last_used_step` に記録し、**同じか古いステップのコードは
  以後受理しない**。判定と記録は 1 文の原子的 UPDATE（`record_totp_step_if_newer`）で行い、同じ
  コードで競合する 2 リクエストの一方だけを通す。ログイン・ポータル・step-up・登録確認のすべてに適用。
- **ログインのタイミングによる利用者列挙を緩和した。** 利用者が見つからない経路では argon2 検証が
  走らず応答が速くなり、存在の有無が測れた。不在時にも実在利用者と同じ argon2 検証を 1 回消費する
  （`domain::password::verify_against_dummy`）。OIDC ログイン・ポータル・管理ログインと、それぞれの
  パスワード変更経路に適用。
- **ES256 署名鍵で `/userinfo`・`/introspect` が壊れる不整合を直した。** アクセストークンはアクティブ
  鍵の algorithm（RS256 / ES256）で署名されるのに、この 2 経路は RS256 決め打ちで検証していた。
  管理トークン検証と同じく鍵の algorithm で検証鍵を組み立てる（`jwt::decoding_key_for`）。

## 2026-09-09（2）（狭い画面で読める形にする。ADR-0048）

- **セキュリティ画面がスマートフォンで読めなかった。** アプリ名 `codelab` が
  **`cod` / `ela` / `b` と 1 文字ずつ縦に割れ**、連携日時は
  `2026-09-04T23:42:07.62` で右端に切れていた。原因は 2 つとも「既にある仕組みを
  使っていなかった」——狭い画面で表をカードへ組み替える `.table-cards` が付いておらず、
  時刻は api が返す RFC 3339 の文字列をそのまま出していた。
- **表 4 つに `.table-cards` と `data-label` を付けた**（セキュリティ・認証器・
  認証ポリシー）。管理コンソールの 13 の表は前から使っていて、この 3 画面だけ
  素の `.table-responsive` で残っていた。**付け忘れを二度と起こさないため**、
  テンプレートを走査して `<table>` を全部確かめるテストを足した。
- **時刻を出す 15 か所すべてを閲覧者のタイムゾーンで描くようにした。**
  `<time class="local-time" datetime="…（UTC のまま）">2026-09-09 11:20 UTC</time>`
  の形で出し、`assets/local-time.js` が中身を差し替える。**中身をサーバー側で
  読める形にしておくのが肝**で、差し替えが起きなくても日時として読める（`UTC` と
  明示するので、自分の時刻と読み違えない）。マイクロ秒の桁も消えた。
- 列見出しの「（UTC）」を外した（値が閲覧者の時刻になった以上、見出しが嘘になる）。
  検索欄の「RFC 3339（UTC）」は**入力の形式**なので残した。
- `<pre>`（JWKS の JSON・traceback）が狭い画面で外へはみ出していたので `.pre-wrap` を足した。
  Bootstrap の `.text-wrap` は `white-space: normal` で**改行が消える**ため使わない。
  操作ボタンのセルの `.text-nowrap` も外した（カードでははみ出す）。
- 管理コンソールの `<html lang>` を出すようにした（日時の整形言語と支援技術のため）。
  利用者向けの `page.html` 側は `MessagePage` / `ErrorPage` が `Messages` を持たない
  ため未対応（Progress の T43）。

## 2026-09-09（発行済みトークンの再発行。ADR-0047）

- **refresh token を落とすことだけを目的にした操作が無かった。** 失効は必ず何かの副作用
  （ログアウト・連携解除・パスワード変更・無効化）として起き、「アプリに渡した鍵が漏れた
  かもしれない」だけの人が選べるものが 1 つも無かった。一番近いパスワード変更は、**漏れて
  いないパスワードを変える**ことになる。管理者側はもっと極端で、ヘルプデスクの道具は
  無効化・パスワード再発行・MFA 解除だけ——どれも利用者の作業を止める。結果として起きるのは
  「様子を見る」で、30 日の refresh token が心当たりを抱えたまま残った。
- **本人と管理者の両方に「トークンの再発行」を足した。** 本人はセキュリティ画面
  （`POST /{tenant_id}/settings/security/reissue-tokens`）、管理者はメンバー一覧の行
  （`POST /{tenant_id}/admin/users/{user_id}/token-reissue`。`idp.users:write`）。
  中身は当人の refresh token の一括失効で、アプリは次のアクセスで SSO 越しに取り直す。
- **触るのはトークンだけ。** セッションも同意も未消費 code も残す。セッションを切ると
  押した本人が締め出され、同意を消すと次回に同意画面が出る——どちらもこの操作が約束して
  いない。code を落としても、生きたセッションを持つ相手は直後に新しい code を取れるので
  守るものが無い（それはセッション失効の役目）。
- テナントで絞らない（ADR-0044 と同じ理由。絞ると見えていないテナントの鍵が生き残る）。
  管理者経路の対象は所属元が要求テナントの利用者に限る。**自分自身にも実行できる**
  （ログイン状態を切らないのでロックアウトにならない。ロック解除と同じ扱い）。
- 失敗したら成功を返さない（fail-closed）。**0 件も成功**とし、落とした本数を返して画面が
  「無効にするトークンはありませんでした」と出し分ける。そのため
  `RefreshTokenRepository::revoke_all_for_user` の戻り値を `()` から行数へ変えた。
- step-up は要求しない。この操作は誰も締め出さないので、セッション失効に本人確認を挟む理由
  （盗んだ側が本人を先に切る）が当てはまらない。
- 監査は `refresh_token.revoked`。トークンの値もハッシュも記録しない。
- 統合テスト（`crates/api/tests/token_reissue.rs`）で実 DB を通し、**更新が断られること**と
  **セッションが残ること**を同じテストで確かめている。

## 2026-09-08（2）（パスワード変更の後始末と、`/revoke` の持ち主確認。ADR-0045 / ADR-0046）

- **自分でパスワードを変えても、他の端末が切れなかった（ADR-0045）。** メールのリンク経由の
  リセットは全セッションと refresh token を落とし、画面も「既存のセッションはすべて
  サインアウトされています」と明言するのに、**設定画面からの変更だけ何もしていなかった**。
  嘘はついていない（何も約束していない）が、「漏れたかもしれないから変える」という動機を
  裏切る。他の SSO セッション・refresh token・未消費の code を落とすようにした。
  **今のセッションだけは残す**——`delete_all_for_user` を呼ぶと変えた本人がその場で
  締め出される（リセット経路が全部消してよいのは、あれが「忘れた人」の経路で手元に
  セッションが無いから）。`delete_all_for_user_except` を足した。
  併せて文言を「パスワードを変更しました。**この端末以外はサインアウトされています。**」に
  した。黙って切ると、利用者は自分がやったことの結果を知らないままになる。
- **`/revoke` がトークンの持ち主クライアントを確かめていなかった（ADR-0046。RFC 7009 §2.1）。**
  認証済みクライアントを `_client` として捨てており、リポジトリも `token_hash` だけで消す
  作りだったので、**同じテナントに登録されたクライアントなら他のクライアントのトークンを
  消せた**。失効の前に `find_by_hash` で引いて持ち主を照合する。
  **不存在（200）と持ち主違い（`unauthorized_client`）は分ける**——一緒くたにエラーへ倒すと
  「トークンが実在するか」を問い合わせる口になり、一緒くたに 200 へ倒すと検証が無いのと
  変わらない。
- どちらも統合テストを新設し、**修正を外すと落ちること**まで確認している。
- 残り: `/revoke` の access_token 側は署名も持ち主も検証していない（Progress の T41）。

## 2026-09-08（セッションを終えたら refresh token も失効させる。ADR-0044）

- **「この端末をログアウト」もログアウトも、refresh token を落としていなかった。**
  `sso_sessions` の行を消すだけだったので、その端末のアプリは更新が通り続けた。
  アクセストークンは既定 15 分で切れるが、**リフレッシュが止まらないので窓が閉じない**
  ——最初の発行から最長 30 日、ログアウトしたはずの端末が動き続けられた。
  短命アクセストークンは、失効が効くことを前提にして初めて意味を持つ。
- 道具は揃っていた。`refresh_tokens.sid` は 0021 で足してあり、`sso_session::sid_of` が
  `session_hash` から導出して発行時に持ち回り rotation でも引き継ぐ。**書き込むだけで
  一度も引いていなかった。** `revoke_all_for_session` と `sid` の索引（0050）を足し、
  `account_security::revoke_session` と `logout::logout` の両方から呼ぶようにした。
- 失効はテナントで絞らない。`SsoSession` はテナントを持たないので、絞ると取りこぼす。
- 失敗の扱いは経路で変えた。セルフサービスの失効は **fail-closed**（成功を返すと
  「切ったつもりで切れていない」を画面が肯定する）、ログアウトは **fail-open**
  （途中で投げると Cookie を消せないまま利用者が宙に浮く。同じ関数の他 2 つと揃えた）。
- 統合テストで「発行 → 更新 → セッション終了 → 更新が断られる」を実 DB で通した。
  **修正を外すと落ちること**も確認している（外すと更新が 200 で通る）。
- 残り: 自分でのパスワード変更はいまも失効させない（Progress の T39。本人のセッションを
  除外する手当てが要るため別件）。`/revoke` のクライアント照合漏れも別件（T40）。
...
- ⚠ **`acr_values` は強制していたのに、満たしたことを RP へ返していなかった。** 認証ポリシーの
  `requested_acr` 条件は本当に効いていて、SSO セッションの復元時にも評価し直している。ところが
  条件は「非空 = いずれかに一致」なので、**ポリシー行へ入力した文字列と RP が送る文字列が
  1 文字ずれると条件が外れ、既定効果（`allow`）へ落ちて単要素のログインが黙って通る**。
  RP には検出する手段が無かった（`acr` も `amr` も ID Token に載っていなかった）。
- **予約語 2 つを定義し、組み込みで解釈するようにした**（`urn:assay:ac:mfa` /
  `urn:assay:ac:single`）。`urn:assay:ac:mfa` を要求されたら、**ポリシーの有無に関わらず** MFA を
  要求する。テナントのポリシー（`require_mfa`）は「RP が要求しなくても強制する」側として残り、
  実効要件は両者の強いほう。方式指定ポリシーと同時に一致した場合は、方式指定へ「第二要素
  いずれか」を足して**両方**満たさせる（片方に丸めると RP の要求がすり抜ける）。
- **ID Token に `acr` / `amr` を載せた。** `acr` は要求の有無に関わらず常に載せる（載っていない
  ことの意味を RP が読み分けられるようにするため）。`amr` は RFC 8176 の語彙で、**強度の判断には
  使わせない**（`fed` の先で何が使われたかは assay も知らない）。強度の 2 値はもともと
  `AuthenticationStrength` として内部にあった派生値で、予約語はその公開名にすぎない。
- **記録なしのときは載せない。** 認証方式を `auth_sessions` / `authorization_codes` /
  `refresh_tokens` へ持ち回す列を足した（`sid` を足したときと同じ形。マイグレーション 0049）。
  本列の導入前に発行された code / refresh token は `acr` / `amr` を名乗らない ——分からないものを
  単一要素と名乗ると嘘になる。
- discovery の `acr_values_supported` に予約語 2 つを、`claims_supported` に `acr` / `amr` を
  載せた。**保証できる値だけを公開する**方針は変えていない（テナントが `requested_acr` 条件へ
  書いた独自の文字列は公開しないし、`acr` としても返さない）。

## 2026-09-02（5）（パスキーが登録できてもログインできなかったのを直した／カードを押せるようにした）

- ⚠ **登録した鍵でログインできない状態だった。** 登録は webauthn-rs の既定のまま
  `residentKey: discouraged` を出しており、認証は `start_discoverable_authentication`
  （`allowCredentials` は空）**しか持たない**。噛み合っていないため、認証器が非 discoverable な
  鍵を作ると**登録は成功するのにログインには一生使えない**。ブラウザは該当なしを
  `NotAllowedError` で返すので、画面には「中止されたか時間切れ」としか出ず原因に辿り着けない。
  登録の options を `residentKey: required` へ上書きした（`mediation` を落としているのと同じ形で、
  webauthn-rs が組んだ値を後から直す）。直列化した JSON で確かめる単体テストを付けた。
  ⚠ **既に登録済みの鍵が非 discoverable だった場合は、登録し直しが要る**（作られた鍵の性質は
  後から変えられない）。
- **一覧のカード（画面が狭いとき）をどこを押しても開けるようにした。** 表では名前のセルだけが
  リンクだが、カードでは項目名と値が縦に並ぶため**どこが押せるのか画面から読めなかった**。
  リンクの当たり判定をカードいっぱいへ広げ、右端に矢印を出した（カード表示のときだけ効かせる。
  素の表で効かせると position を持つ祖先まで当たり判定が広がる）。操作列のボタンは覆いより前に置く。
