...
- **ログイン識別子の登録簿を導入した（AP8。ADR-0025・migration 0029）。** ログイン欄に入力できる
  ...
  **表示値と正規化値**・有効/無効を持たせ、テナント管理者が
  ...
  - **解決は「登録簿 → `users.preferred_username`」の順**（expand フェーズ）。主たる識別子の移送
    （contract）は分けた。失敗すると誰もログインできなくなる変更で、登録簿の導入と同じ回に
    載せない（AP9 / ADR-0022 と同じ分け方）。ログイン 4 経路は
    `UserRepository::find_by_login_identifier` の 1 本に寄せ、どこを引いたかは実装に閉じた。
  - **写しの同期を 1 か所に閉じた。** `users` 側だけを変えると**変更前のユーザー名でログインできる**
    （登録簿を先に引くため）。利用者作成（管理者作成・自己登録）とプロフィール編集の両方を
    `PrimaryLoginIdentifierSync` へ通し、一意性の事前チェックも解決経路と同じ引き方に揃えた
    （`users` だけを見ると、別名として登録済みの値を素通しにする）。
  - **`users.email` は backfill しない。** 取り込むとマイグレーションを当てた瞬間からメールで
    ログインできてしまい、認証の入り口が黙って広がる。`email` 種別を明示的に足したときだけ有効。
  - 追加できる値かは**実際の解決経路で引いてみて**判定する（他人に解決される値を拒否）。メール種別は
    加えて `users.email` も見る（メールは現状ログイン経路に無いが、他人の連絡先でなりすませる）。
