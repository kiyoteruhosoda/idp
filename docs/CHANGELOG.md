## 2026-08-25（2）（機械の登録: 用途を先に選ばせ、grant_types は導出する）

- **M2M クライアントを redirect_uri なしで登録できるようにした（ADR-0032 決定 2）。** ADR-0030 で
  `private_key_jwt` を入れたのに、**登録側が種別を問わず redirect_uri を必須にしていたため機械を
  登録できなかった**。手順書に書いた `"redirect_uris": []` は実際には通らず、ブラウザのリダイレクト
...
- **登録フォームは「用途」を最初に選ばせ、以降の入力欄をそれで変えるようにした（決定 3）。**
  以前は最下部のチェックボックスが最上部の入力欄の要否を決めていた。用途に「機械」を選ぶと
...
- 両方の grant を持つ既存クライアントには、編集画面でだけ選択肢「両方（既存の設定）」を出す
  （決定 4）。丸めると、編集画面を開いて保存しただけで `client_credentials` が黙って外れる。
