## 2026-09-16（8）（アプリを使う主体に、サービスアカウントを並べる）

- **アプリを使う主体を「人」と「サービスアカウント」の 2 種類にした**（`0058`。ADR-0059 の決定 5・6）。
...
## 2026-09-16（7）（アプリの名乗りと、名簿の self）
...
- **アプリの名乗り（`application_bindings`）を 4 種類にした**（`0057`）——ログイン用（`oidc`）・`saml`・
