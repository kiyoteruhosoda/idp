...
- ⚠ **1 人の画面が 500 を返していた（E2E が捕まえた）。** `Path<Uuid>` を 1 つだけで受けており、
  経路 `/{tenant_id}/admin/members/{user_id}` の**先頭のテナント ID を取っていた**ため、必ず
  「メンバーではない」になっていた。同じ経路の `revoke_member` / `update_member_status` は
  最初から `Path<(String, String)>` で 2 つ受けている。**新しい口に試験を付けていなかったのが
  見落とした理由**なので、`one_member_can_be_fetched_by_id` を足した。
