## 2026-08-27（クライアント新規登録の既定の用途をシステム用にした）

- **管理コンソールのクライアント新規登録で、用途の初期選択を「システムが API を呼ぶ（利用者不在）」に
  した（ADR-0038）。** 選択肢の並びも既定を先頭へ置く形に揃えた（認証方式と同じ規則。ADR-0036）。
  画面を開いた時点でリダイレクト URI と client type の欄は隠れており、ブラウザログイン用の RP を
  登録するときに用途を選び直す。
- **api の省略時の挙動は変えていない。** `POST /{tenant_id}/admin/clients` は従来どおり
  `allow_client_credentials` の省略を `false` と読む。見えない省略値に能力を付けないため、
  画面の初期選択とは意図的に揃えていない（ADR-0038 決定 2）。
