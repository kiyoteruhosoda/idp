## 2026-07-24（web に 403/404/500 の共通エラーページを追加した）

- **HTTP エラーページ（403 / 404 / 500）を web に追加した**: ステータスコードを大きく表示し、タイトルと
  説明文を添える共通テンプレート（`error_page.html` / `ErrorPage`）を新設した。どのルートにも一致しない
  リクエストは axum 既定の空応答ではなく 404 エラーページを返すよう、ルータへ `fallback`（`handlers::errors`）を
  配線した。文言は i18n（`error-403/404/500-*`、ja/en）で管理し、テナント文脈を持たない経路でも
  `Accept-Language`（＋`lang` Cookie）から表示言語を決める。`handlers::errors::page(status, headers)` は
  各ハンドラからも再利用できる。
