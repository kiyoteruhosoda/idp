...
- **root でも認証できない場合（`.env` ごと別物）を別の診断に分けた。** この状態では
  ...
  「`reset`」を提示して停止する。
- **`.env` の値を Compose と同じ解釈（対のクォートを外す）で読むようにした。** `MARIADB_PASSWORD="…"` の
  ように引用符付きで書かれていると、Compose はクォートを外した値でコンテナを作るのに `deploy.sh` は
  クォート込みで認証していたため、正しい `.env` でもプリフライトが偽陽性で落ちていた。
