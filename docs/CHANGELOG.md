## 2026-07-25（API エラーメッセージを多言語化した。MT19）

- **利用者向けメッセージを Application 層の「文言」から「翻訳キー」へ変えた**（`domain/message.rs` の
  `UserMessage`）。Application 層は言語を知らずにキーと埋め込み引数だけを返し、Presentation 層が
  `Accept-Language` に応じて訳す。従来は英語の内部文言（`"only active guest memberships can be
  suspended"` 等）がそのまま API 応答の `message` に出ていた。
- **エラーコード（応答の `error`）は言語不変**を維持し、`message` だけを訳す。クライアントはコードで
  分岐するため、ここが言語で揺れると多言語化が API の互換性を壊す。
- 埋め込み引数（不正な scope 名・リダイレクト URI・パスワード長など）は Fluent の `{ $name }` として
  渡し、どの値が問題なのか分かるメッセージにした。
- 翻訳キーは `keys::ALL` に集約し、**両言語に訳が揃っていること**をテストで強制する。`get` は
  フェイルソフト（未定義キーはキー名を返す）のため、訳の抜けは強制しないと応答にキー名が出るまで
  気付けない。
- 対象外は従来どおり運用情報（ログ・監査ログ・例外詳細）で、各エラーの `Internal` は英語の
  `String` のまま（CLAUDE.md「多言語化の対象範囲」）。
